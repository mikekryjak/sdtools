...
ds2 = ds2.isel(
    pos=slice(1, -1)
)  # Crop off outer guard cells, keep inners for target values
...
def tanh(x, x0):
    return (np.exp(2.0 * (x - x0)) - 1.0) / (np.exp(2.0 * (x - x0)) + 1.0)


# ----- Source up to x-point
sourceVolume = ds["dv"].values[1:-1]
sourceVolume[np.argmin(np.abs(ds["pos"].values - 50.0)) :] = 0
sourceVolume = np.trapz(sourceVolume, ds["pos"].values[1:-1])

# ----- Tanh Pump
pumpRegion = (0.5 * tanh(ds["pos"].values[1:-1], 62.5) + 0.5) * (
    -0.5 * tanh(ds["pos"].values[1:-1], 87.5) + 0.5
)
...
options = BoutData(
    filePath, yguards=True, info=False, strict=True, DataFileCaching=False
)["options"]
...
if guardRep:
    Q_t_electrons = (
        gamma_e * ds["Ne"][:, -1] * q_e * ds["Te"][:, -1] * ds["Ve"][:, -1] * csArea[-1]
    )  # ds["J"]/np.sqrt(ds["g_22"])#ds["da"][0,-1]
    Q_t_ions = (
        gamma_i
        * ds["Nd+"][:, -1]
        * q_e
...
# ----- Total Input Power
totalInputPow = (3.0 / 2.0) * (
    (ds["Pe_src"] * ds["dv"]).sum("pos") + (ds["Pd+_src"] * ds["dv"]).sum("pos")
)  # Pe_src is in Pa.s^-1 which is J.m^-3.s^-1 = W.m^-3
# ----- Impurity Radiation
totalRadArPow = (ds["Rar"] * ds["dv"]).sum("pos")
# ----- Hydrogenic Radiation
totalRadHexPow = np.abs(
    (ds["Rd+_ex"] * ds["dv"]).sum("pos")
)  # Needs to be negative because of output conventions
totalRadHrecPow = np.abs((ds["Rd+_rec"] * ds["dv"]).sum("pos"))
# ----- CX
totalCXKinetic = 0.5 * ((abs(ds["Fdd+_cx"]) * abs(ds["Vd+"])) * ds["dv"]).sum(
    "pos"
)  # E = 0.5* mv*v ???
...
print(
    "For input power: %.3e , total time needed to fully ionised neutrals (for zero losses) = %.3e ms"
    % (totalInputPow.values[-1], timeToIonise * 1e3)
)
