...
AA_e = 1.0 / 1836  # Electron atomic mass number
...
print(
    f"Total ion particle source:  {total_ion_source}  (check: {particle_source + ionization + recombination + feedback_source}"
)
...
print(
    f"Total neutral particle source: {total_neutral_source} (check: {neutral_source - ionization - recombination + neutral_recycle})"
)
...
print(
    f"  Sound speed: {Cs0 * np.sqrt((sheath_ion_polytropic * ti_sheath + Zi * te_sheath) / AA)} m/s"
)
...
ion_energy_flux = (
    gamma_i * (Pnorm * Cs0) * ni_sheath * ti_sheath * vi_sheath * area_sheath
)
electron_energy_flux = (
    gamma_e * (Pnorm * Cs0) * ni_sheath * te_sheath * vi_sheath * area_sheath
)

ion_convection = 2.5 * (Pnorm * Cs0) * ni_sheath * ti_sheath * vi_sheath * area_sheath
ion_kinetic = (
    (Pnorm * Cs0) * 0.5 * vi_sheath**2 * AA * ni_sheath * vi_sheath * area_sheath
)

electron_convection = (
    2.5 * (Pnorm * Cs0) * ni_sheath * te_sheath * vi_sheath * area_sheath
)
electron_kinetic = (
    (Pnorm * Cs0) * 0.5 * vi_sheath**2 * AA_e * ni_sheath * vi_sheath * area_sheath
)
...
print(
    f"Total power loss: {(ion_energy_flux + electron_energy_flux - recycle_heating - R_ex - R_rec) * 1e-6} MW"
)
print(f"  |- Ions:              {ion_energy_flux * 1e-6} MW")
print(f"      |- Convection          {ion_convection * 1e-6} MW")
print(f"      |- Kinetic energy      {ion_kinetic * 1e-6} MW")
print(
    f"      |- Conduction          {(ion_energy_flux - ion_kinetic - ion_convection) * 1e-6} MW"
)
...
print(
    f"      |- Conduction          {(electron_energy_flux - electron_kinetic - electron_convection) * 1e-6} MW"
)
