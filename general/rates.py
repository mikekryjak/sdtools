...
def loglog_cooling_curve_fit(
    ...
    coeffs = np.polyfit(dftrim["logT"], dftrim["logLz"], deg=10)[
        ::-1
    ]  # polyfit gives coeffs in opposite order to what polynomial needs
    ...
    if plot_output is True:
        ...
        for ax in axes:
            ...
            ax.plot(
                df["T"], df["Lz"], label="All data", marker="x", lw=0, c="grey", ms=4
            )
            ax.plot(
                dftrim["T"],
                dftrim["Lz"],
