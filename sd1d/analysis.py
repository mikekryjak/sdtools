...
class Case:
    ...
    def load_data(self):
        self.boutdata = BoutData(
            self.casepath, yguards=True, strict=True, DataFileCaching=False
        )
    ...
    def extract_variables(self, tind=-1):
        ...
        var_collect = [
            ...
            "Fiz",
            "Fcx",
            "Fel",  # Momentum source/sinks to neutrals
            "R",
            "Rrec",
            "Riz",
            "Rzrad",
            "Rex",  # Radiation, energy loss from system
            "E",
            "Erec",
            "Eiz",
            "Ecx",
            "Eel",  # Energy transfer between neutrals and plasma
            "Pe",
            "Pd",
            "Pd+",
            "Ve",
        ...
        if self.hermes:
            ...
            self.norm_data["NVi"] = (
                self.norm_data["NVd+"] * constants("mass_p") / (constants("mass_p") * 2)
            )  # Originally [kgm2s-1], then divide by mass_i to get flux.
        ...
        if self.evolve_nvn:
            if self.hermes:
                ...
                self.norm_data["Vd"] = (
                    self.norm_data["NVd"] / self.norm_data["Nd"]
                )  # AA
        ...
        if self.ion_eqn:
            self.norm_data["Te"] = (
                self.norm_data["Pe"] / self.norm_data["Ne"]
            )  # Electron temp
            self.norm_data["Ni"] = self.norm_data["Nd+"]
        else:
            self.norm_data["Te"] = (
                0.5 * self.norm_data["P"] / self.norm_data["Ne"]
            )  # Electron temp
    ...
    def process_variables(self):
        ...
        self.Pnorm = (
            self.Nnorm * self.Tnorm * constants("q_e")
        )  # Converts P to Pascals. 1.602e-19 is proton charge in C
        self.Snorm = (
            self.Nnorm * self.Omega_ci
        )  # Normalisation for S: plasma density sink (m-3s-1)
        if self.hermes:
            self.Fnorm = (
                (constants("mass_p")) * self.Nnorm * self.Cs0 * self.Omega_ci
            )  # Everything normalised to mass_p not mass_i in Hermes-3
        else:
            self.Fnorm = (
                (constants("mass_p") * 2) * self.Nnorm * self.Cs0 * self.Omega_ci
            )  # [kgm-2s-1] Plasma momentum sink normalisation
        self.Enorm = (
            constants("q_e") * self.Nnorm * self.Tnorm * self.Omega_ci
        )  # [Wm-3] Plasma energy sink normalisation
        ...
        # Sheath stats
        self.flux_out = (
            norm["NVi"][-1] * self.J / np.sqrt(self.g_22) * self.Omega_ci * self.Nnorm
        )[
            0
        ]  # Sheath flux in [s-1]. J/g_22 is cross-sectional area (J*dy)/(sqrt(g_22)*dy) = Volume/length
        ...
        dnorm["Cs"] = np.sqrt(
            constants("q_e") * (dnorm["Te"] * 2) / (constants("mass_p") * 2)
        )
    ...
    def plot_residuals(
        ...
        num_timesteps = len(
            collect("Ne", path=self.casepath, yguards=True, info=False, strict=True)[
                :, 0, 0, 0
            ]
        )
        ...
        for param in params:
            try:
                hist = collect(
                    param, path=self.casepath, yguards=True, info=False, strict=True
                )[:, 0, 1:-1, 0]
            except:
        ...
        ax.set_title(
            f"{self.casename} || Normalised residual plot || smoothing: {smoothing}",
            fontsize=12,
        )
        ax.set_yscale("log")
        ax.set_ylim(1e-10, 1)
    ...
    def plot_density_controller(
        ...
        for i, case in enumerate(plot_cases):
            ...
            hist_Ne = collect("Ne", path=self.casepath, yguards=True, info=False)[
                :, 0, 1:-1, 0
            ]

            try:
                if self.hermes:
                    hist_NeSource = collect(
                        "Sd+_src", path=self.casepath, yguards=True, info=False
                    )[:, 0, 1:-1]
                else:
                    hist_NeSource = collect(
                        "NeSource", path=self.casepath, yguards=True, info=False
                    )[:, 0, 1:-1]
            except:
            ...
            for t in range(num_timesteps[case]):
                ...
                try:
                    h_NeSource[case].append(
                        replace_guards(hist_NeSource[t, :])[0] * Nnorm * Omega_ci
                    )
                except:
                    pass

            if self.hermes:
                intended_Ne[case] = BoutData(self.casepath)["options"]["d+"][
                    "density_upstream"
                ]
            else:
                intended_Ne[case] = BoutData(self.casepath)["options"]["sd1d"][
                    "density_upstream"
                ]
    ...
    def get_htransfer(self):
        ...
        self.data["pos_boundary"], self.data["convect_ke"] = ke_convection(
            self.pos, self.data["Ne"], self.data["Vi"]
        )
        _, self.data["convect_therm"] = ke_convection(
            self.pos, self.data["P"], self.data["Vi"]
        )
        ...
        _, self.data["conduction_SH_script"] = heat_conduction(
            self.pos, self.data["Te"]
        )
    ...
    def heat_balance(self, verbose=False, override=False):
        if self.hermes and override == False:
            ...
            self.hflux_sheath = (
                self.Tt * constants("q_e") * self.flux_sheath * self.sheath_gamma * 1e-6
            )

            self.hflux_E = np.trapz((self.data["E"] * self.dV)[1:-1]) * 1e-6
            self.hflux_R = np.trapz((self.data["R"] * self.dV)[1:-1]) * 1e-6
            self.hflux_F = (
                np.trapz((self.data["F"][1:-1] * self.data["Vi"][1:-1] * self.dV[1:-1]))
                * 1e-6
            )

            self.hflux_out = (
                self.hflux_sheath + self.hflux_E + self.hflux_R + self.hflux_F
            )
    ...
    def heat_balance_hermes(self):
        ...
        d = BoutData(
            self.casepath, yguards=True, info=False, strict=True, DataFileCaching=False
        )["outputs"]
        o = BoutData(
            self.casepath, yguards=True, info=False, strict=True, DataFileCaching=False
        )["options"]
        ...
        AA = 2
        NVi = (
            extract(d["NVd+"]) * Xnorm * mass_p / mass_i
        )  # Originally [kgm2s-1], then divide by mass_i to get flux.
        ...
        # From saved output:
        case_i_hflux_add = (
            -1 * d["Ed+_sheath"].squeeze()[-1, :][-3] * dV[-3] * Enorm * 1e-6
        )
        case_e_hflux_add = (
            -1 * d["Ee_sheath"].squeeze()[-1, :][-3] * dV[-3] * Enorm * 1e-6
        )
        ...
        print("\n- Ions ---------------")
        print(
            f"- Required additional power for chosen gamma = {sheath_i_hflux_additional:.2f} [MW]"
        )
        print(f"- Additional cooling power in code = {case_i_hflux_add:.2f} [MW]")
        print("\n- Electrons ---------------")
        print(
            f"- Required additional power for chosen gamma = {sheath_e_hflux_additional:.2f} [MW]"
        )
        ...
        print(
            f"- Difference = {total_in + total_out:.2f} [MW] or {(total_in + total_out) / total_in:.1%}"
        )
    ...
    def mass_balance_hermes(self):
        ...
        if "Sd+_src" in d.keys():
            input_source = np.trapz(
                x=pos[2:-2],
                y=d["Sd+_src"].squeeze()[:, 2:-2] * J[2:-2] * d["Cs0"] * d["Nnorm"],
            )
        else:
            input_source = np.zeros_like(sheath_ion_flux)

        if "Sd_src" in d.keys():
            input_n_source = np.trapz(
                x=pos[2:-2],
                y=d["Sd_src"].squeeze()[:, 2:-2] * J[2:-2] * d["Cs0"] * d["Nnorm"],
            )
        else:
        ...
        if "Sd+_iz" in d.keys():
            iz_source = np.trapz(
                x=pos[2:-2],
                y=d["Sd+_iz"].squeeze()[:, 2:-2] * J[2:-2] * d["Cs0"] * d["Nnorm"],
            )
        else:
        ...
        if "Sd+_rec" in d.keys():
            rec_source = np.trapz(
                x=pos[2:-2],
                y=d["Sd+_rec"].squeeze()[:, 2:-2] * J[2:-2] * d["Cs0"] * d["Nnorm"],
            )
        else:
        ...
        if "SNd+" in d.keys():
            alt_source = np.trapz(
                x=pos[2:-2],
                y=d["SNd+"].squeeze()[:, 2:-2] * J[2:-2] * d["Cs0"] * d["Nnorm"],
            )
        else:
        ...
        if "ddt(Nd+)" in d.keys():
            ddt_int = np.trapz(
                x=pos[2:-2],
                y=d["ddt(Nd+)"].squeeze()[:, 2:-2] * J[2:-2] * d["Cs0"] * d["Nnorm"],
            )
        ...
        # ----- Totals
        total_ions = np.trapz(
            x=pos[2:-2], y=d["Nd+"].squeeze()[:, 2:-2] * J[2:-2] * d["Nnorm"]
        )
        total_neutrals = np.trapz(
            x=pos[2:-2], y=d["Nd"].squeeze()[:, 2:-2] * J[2:-2] * d["Nnorm"]
        )
        total_particles = total_ions + total_neutrals

        case_ion_source = np.trapz(
            x=pos[2:-2],
            y=d["SNd+"].squeeze()[:, 2:-2] * J[2:-2] * d["Cs0"] * d["Nnorm"],
        )
        ...
        ax2 = ax.twinx()
        ax2.plot(
            t, frac_balance, lw=2, alpha=0.3, ls="-", c="magenta", label="Imbalance"
        )
        ax2.set_ylim(-1, 1)
        ...
        print(f"\n- Difference:")
        print(
            f"---> {total_balance[-1]:.3E} [s-1] ({total_balance[-1] / total_in[-1]:.3%})"
        )
    ...
    def mass_balance(
        ...
        h["Ne"] = collect("Ne", path=path_case, yguards=True, info=False, strict=True)[
            :, 0, 1:-1, 0
        ]
        d["Nnorm"] = collect("Nnorm", path=path_case)
        d["Cs0"] = collect("Cs0", path=path_case)
        d["Omega_ci"] = collect("Omega_ci", path=path_case)
        d["dy"] = collect("dy", path=path_case, yguards=True, info=False, strict=True)[
            0, 1:-1
        ]
        d["dy_full"] = collect(
            "dy", path=path_case, yguards=True, info=False, strict=True
        )[0, :]
        d["J_full"] = collect(
            "J", path=path_case, yguards=True, info=False, strict=True
        )[0, :]

        if self.hermes:
            h["Vi"] = collect(
                "Vd+", path=path_case, yguards=True, info=False, strict=True
            )[:, 0, 1:-1, 0]
            h["NVi_original"] = collect(
                "NVd+", path=path_case, yguards=True, info=False, strict=True
            )[:, 0, 1:-1, 0]
            ...
            if "upstream_density_feedback" in self.options.keys():
                Nu_target = self.options["upstream_density_feedback"][
                    "density_upstream"
                ]
            else:
                Nu_target = np.nan
            ...
            h["Vi"] = collect(
                "Vi", path=path_case, yguards=True, info=False, strict=True
            )[:, 0, 1:-1, 0]
            h["NVi_original"] = collect(
                "NVi", path=path_case, yguards=True, info=False, strict=True
            )[:, 0, 1:-1, 0]
        ...
        try:
            h["S"] = collect(
                "S", path=path_case, yguards=True, info=False, strict=True
            )[:, 0, 1:-1, 0]
            sink = True
        except:
            sink = False
        ...
        try:
            h["Nn"] = collect(
                "Nn", path=path_case, yguards=True, info=False, strict=True
            )[:, 0, 1:-1, 0]
            h["Nn_full"] = collect(
                "Nn", path=path_case, yguards=True, info=False, strict=True
            )[:, 0, :, 0]
        ...
        NeSource = collect(
            "NeSource", path=path_case, yguards=True, info=False, strict=True
        )
        ...
        h["NVi"] = (
            h["Ne"] * h["Vi"]
        )  # NVi calculated like this because the SD1D NVi variable is already guard replaced and would cause confusion
        h["NVi_dn"] = (
            h["NVi"] * d["J"] / np.sqrt(d["g_22"]) * d["Omega_ci"] * d["Nnorm"]
        )
        h["NVi_original_dn"] = (
            h["NVi_original"] * d["J"] / np.sqrt(d["g_22"]) * d["Omega_ci"] * d["Nnorm"]
        )
        ...
        if evolving_source == True:
            ...
            h["NeSource"][:, -1] = 0
            h["NeSource_dn"] = (
                h["NeSource"] * d["dy"] * d["J"] * d["Omega_ci"] * d["Nnorm"]
            )
        ...
        if evolving_source == False:
            d["NeSource"][0] = 0
            d["NeSource_dn"] = (
                d["NeSource"] * d["dy"] * d["J"] * d["Omega_ci"] * d["Nnorm"]
            )
        ...
        if neutrals:
            if nloss_guardcells:
                ...
                h["nloss_dn"] = (
                    nloss * h["Nn_full"] * d["Nnorm"] * d["dy_full"] * d["J_full"]
                )
            else:
        ...
        d["N_int_ma"] = np.convolve(
            d["N_int_trim"], np.ones(ma_period) / ma_period, mode="valid"
        )
        ...
        if verbosity == True:
            ...
            print(
                "- Average particle change per timestep: {:.3E}".format(
                    d["mass_gain_rate"]
                )
            )
            print("--------------------------------")
            ...
            print(
                ">>Mass change rate as fraction of total particles: {:.3E}".format(
                    d["mass_rate_frac"]
                )
            )
            print("--------------------------------")
        ...
        if plot == True:
            if error_only == True:
                ...
                ax.plot(
                    ...
                    color="green",
                    linewidth=2,
                    label=f"Nu error, final={Nu_error[-1]:.2%}",
                    linestyle="solid",
                )
                ax.plot(
                    flux_imbalance,
                    color="red",
                    linewidth=2,
                    label=f"Mass error, final={flux_imbalance[-1]:.2%}",
                    linestyle="solid",
                )
                ax.plot(
                    frecycle_error,
                    color="red",
                    linewidth=2,
                    label=f"Recycle error, final={frecycle_error[-1]:.2%}",
                    linestyle="dashed",
                )
                ...
                fig, ax = plt.subplots(figsize=(6, 6))
                ax.plot(
                    d["NVi_out_dn"] + (d["nloss_int_dn"]),
                    color="black",
                    linewidth=2,
                    label=f"Sheath+nloss sink",
                    linestyle="solid",
                )
                ax.stackplot(
                    range(len(d["NVi_out_dn"])),
                    d["S_int_dn"] * -1,
                    d["NeSource_int_dn"],
                    labels=["Ionisation source", "Flux source"],
                    alpha=0.5,
                )
            ...
            if additional_plots == True:
                ...
                ax.set_title(
                    "Mass balance error for {}".format(self.casename), fontsize=10
                )
                ax.tick_params(axis="both", which="major", labelsize=12)
                ax.tick_params(axis="both", which="minor", labelsize=12)
                ...
                ax.set_title(
                    "Total mass history for {}".format(self.casename), fontsize=10
                )
                ax.tick_params(axis="both", which="major", labelsize=12)
                ax.tick_params(axis="both", which="minor", labelsize=12)
        ...
        if output == True:
            return {
                "NeSource": flux_source[-1],
                "S": flux_sink[-1],
                "NVi": flux_sheath[-1],
                "input_flux": flux_intended,
                "nloss": flux_nloss[-1],
                "input_frecycle": frecycle_intended,
                "actual_frecycle": frecycle[-1],
                "recycle_error": frecycle_error[-1],
                "imbalance": flux_imbalance[-1],
                "mass_rate_frac": d["mass_rate_frac"],
            }
    ...
    def sheath_boundary_simple(case):
        ...
        d = BoutData(
            casepath, yguards=True, info=False, strict=True, DataFileCaching=False
        )["outputs"]
        o = BoutData(
            casepath, yguards=True, info=False, strict=True, DataFileCaching=False
        )["options"]
        ...
        print("")
        print(
            f"- Ratio of NVi/Ni/Vi/mass_p at guard = {NVi_ip / Vi_ip / Ne_ip / mass_p}"
        )
        print(
            f"- Ratio of NVi/Ni/Vi/mass_p at sheath = {nvisheath / visheath / nesheath / mass_p}"
        )
        print(
            f"- Note that the ratio is satisfied at the sheath, and the guard cell diverges due to extrapolation."
        )
        print("")
        ...
        q_i = (
            gamma_i - 2.5
        ) * tisheath * q_e - 0.5 * C_i_sq * mass_i * nisheath * visheath * 1e-6

        print(
            f"- Additional ion cooling in BC: {q_i:.2f} [MW] Model check: x{q_i / i_hflux_add:.2f}"
        )
        ...
        phi[i] = tesheath * np.log(
            np.sqrt(tesheath * q_e / (mass_e * np.pi * 2))
            * (1 - Ge)
            * nesheath
            / ion_sum[i]
        )
        ...
        print(
            f"- Current in last cell: x{ion_sum[i] / case_J[-3]:.1f} which is {ion_sum[i]:.2E} [A]"
        )
        print(
            f"- Potential in last cell: x{phi[i] / case_phi[-3]:.1f} which is {phi[i]:.2E} [A]"
        )
        ...
        tesheath = 0.5 * (Te_ip + Te[i])
        phisheath = np.clip(
            0.5 * (phi_ip + phi[i]), 0, None
        )  # Electron saturation at phi = 0

        vesheath = (
            np.sqrt((tesheath) * q_e / (2 * np.pi * mass_e))
            * (1 - Ge)
            * np.exp(-phisheath / tesheath)
        )
    ...
    def plot(
        ...
        for list_params in vars:
            ...
            for i, ax in enumerate(axes):
                ...
                ax.set_ylabel(
                    "{} ({})".format(lib[param]["name"], lib[param]["unit"]),
                    fontsize=11,
                )
    ...
    def check_rec(self):
        ...
        Erec = (
            self.data["Erec"] * 1e-6 / (3 / 2)
        )  # TODO figure out why on earth the 3/2 is needed
        Frec = self.data["Frec"]
    ...
    def check_rad(self):
        ...
        for i, _ in enumerate(pos):
            Rex_amj[i] = (
                rtools.amjuel_2d("H.10 2.1.5", Te[i], Ne[i])
                * Ne[i]
                * Nn[i]
                * constants("q_e")
                * 1e-6
            ...
            Rrec_amj[i] = (
                (
                    -rtools.amjuel_2d("H.10 2.1.8", Te[i], Ne[i]) * Ne[i] * Ne[i]
                    + rtools.amjuel_2d("H.4 2.1.8", Te[i], Ne[i]) * Ne[i] * Ne[i] * 13.6
                )
                * constants("q_e")
                * 1e-6
            )
            ...
            Rrec_base[i] = (
                rtools.amjuel_2d("H.4 2.1.8", Te[i], Ne[i])
                * Ne[i]
                * Ne[i]
                * 13.6
                * constants("q_e")
            ...
            Rrec_rad[i] = (
                rtools.amjuel_2d("H.10 2.1.8", Te[i], Ne[i])
                * Ne[i]
                * Ne[i]
                * constants("q_e")
                * 1e-6
    ...
    def check_cx(self):
        ...
        if self.hermes:
            ...
            print(
                "This is a Hermes case. Comparing to H.2 3.1.8 low E limit AMJUEL rate."
            )
            for i, _ in enumerate(pos):
    ...
    def check_dn(self):
        ...
        ax.plot(
            pos[zoom_idx:-1], Dn_calc[zoom_idx:-1], label="Dn (check)", ls=":", c="r"
        )
...
class CaseDeck:
            ...
            for file in files:
                if ".dmp" in file:
                    ...
                    if explicit != []:
                        ...
                        if keys != []:
                            if (
                                any(x in case for x in keys)
                                and any(x in case for x in skip) == False
                            ):
                                self.casepaths[case] = root
    ...
    def get_stats(self):
        ...
        for casename in self.casenames:
            ...
            if case.hermes:
                self.stats.loc[casename, "initial_dens"] = (
                    case.options["Nd+"]["function"] * case.Nnorm
                )
                self.stats.loc[casename, "line_dens"] = (
                    case.options["Nd+"]["function"] * case.Nnorm
                )
            else:
                self.stats.loc[casename, "initial_dens"] = (
                    case.options["Ne"]["function"] * case.Nnorm
                )
                self.stats.loc[casename, "line_dens"] = (
                    case.options["Ne"]["function"] * case.Nnorm
                )
    ...
    def get_heat_balance(self):
        ...
        for casename in self.casenames:
            ...
            self.heat_balance.loc[casename, "hflux_imbalance_ratio"] = (
                case.hflux_imbalance / case.hflux_source
            )
...
class Atomics:
    ...
    def pop_ex(self, Te, Ne):
        ...
        for i, n in enumerate(range(2, 7)):
            R[n] = (
                amjuel_2d([Te], Ne, amjuel_data[rates[i]], pop_ratio=True)
                * einsteins[i]
                * bandgaps[i]
            )
...
class AMJUEL:
    ...
    def read_amjuel_2d(self, path):
        ...
        rate = pd.concat(
            [
                rate.loc[2:10, :3].reset_index(),
                rate.loc[13:21, :3].reset_index(),
                rate.loc[24:32, :3].reset_index(),
            ],
            axis=1,
            ignore_index=True,
        )
    ...
    def amjuel_2d(
        ...
        if mode == "default":
            for E_index in data.columns:
                for T_index in data.index:
                    rate = rate + data.loc[T_index, E_index] * (
                        np.log(E) ** E_index
                    ) * (np.log(Te) ** T_index)
        ...
        if mode == "coronal":
            E_index = 0
            for T_index in data.index:
                rate = rate + data.loc[T_index, E_index] * (np.log(E) ** E_index) * (
                    np.log(Te) ** T_index
                )
        ...
        if mode == "Python":
            ...
            for i, col in enumerate(data.columns):
                ...
                for i in range(3):
                    print(
                        "{:.12E}, {:.12E}, {:.12E},".format(
                            data.loc[k, col], data.loc[k + 1, col], data.loc[k + 2, col]
                        )
                    )
                    k += 3
                print("],", end="")
    ...
    def get_amjuel_data(self):
        ...
        self.amjuel_data["H.4 2.1.5JH"] = self.read_amjuel_2d(
            os.path.join(onedrive, r"Project\Atomicrates\H.4 2.1.5JH.csv")
        )  # Ionisation, L.C.Johnson
        self.amjuel_data["H.4 2.1.5"] = self.read_amjuel_2d(
            os.path.join(onedrive, r"Project\Atomicrates\H.4 2.1.5.csv")
        )  # Ionisation, Sawada
        self.amjuel_data["H.4 2.1.5o"] = self.read_amjuel_2d(
            os.path.join(onedrive, r"Project\Atomicrates\H.4 2.1.5o.csv")
        )  # Ionisation, L.C.Johnson, Ly-opaque

        # Excitation energy weighted rate
        self.amjuel_data["H.10 2.1.5JH"] = self.read_amjuel_2d(
            os.path.join(onedrive, r"Project\Atomicrates\H.10 2.1.5JH.csv")
        )  # Excitation, L.C.Johnson
        self.amjuel_data["H.10 2.1.5"] = self.read_amjuel_2d(
            os.path.join(onedrive, r"Project\Atomicrates\H.10 2.1.5.csv")
        )  # Excitation, Sawada
        self.amjuel_data["H.10 2.1.5o"] = self.read_amjuel_2d(
            os.path.join(onedrive, r"Project\Atomicrates\H.10 2.1.5o.csv")
        )  # Excitation, L.C.Johnson, Ly-opaque
        self.amjuel_data["H.10 2.1.8"] = self.read_amjuel_2d(
            os.path.join(onedrive, r"Project\Atomicrates\H.10 2.1.8.csv")
        )  # Recombination, Sawada

        # Recombination particle source rate
        self.amjuel_data["H.4 2.1.8"] = self.read_amjuel_2d(
            os.path.join(onedrive, r"Project\Atomicrates\H.4 2.1.8.csv")
        )  # Sawada, radiative + 3 body
        self.amjuel_data["H.4 2.1.8a"] = self.read_amjuel_2d(
            os.path.join(onedrive, r"Project\Atomicrates\H.4 2.1.8a.csv")
        )  # Johnson, radiative only
        self.amjuel_data["H.4 2.1.8b"] = self.read_amjuel_2d(
            os.path.join(onedrive, r"Project\Atomicrates\H.4 2.1.8b.csv")
        )  # Johnson, three body only
        self.amjuel_data["H.4 2.1.8JH"] = self.read_amjuel_2d(
            os.path.join(onedrive, r"Project\Atomicrates\H.4 2.1.8JH.csv")
        )  # Johnson, raditive + 3 body
        self.amjuel_data["H.4 2.1.8o"] = self.read_amjuel_2d(
            os.path.join(onedrive, r"Project\Atomicrates\H.4 2.1.8o.csv")
        )  # Johnson, Ly-opaque

        # Charge exchange
        self.amjuel_data["HYDHEL H.3 3.1.8"] = self.read_amjuel_2d(
            os.path.join(onedrive, r"Project\Atomicrates\HYDHEL H.3 3.1.8.csv")
        )  # Hydhel improved fit
        self.amjuel_data["H.3 3.1.8"] = self.read_amjuel_2d(
            os.path.join(onedrive, r"Project\Atomicrates\H.3 3.1.8.csv")
        )  # Same as hydhel equivalent
        self.amjuel_data["H.3 3.1.8org"] = self.read_amjuel_2d(
            os.path.join(onedrive, r"Project\Atomicrates\H.3 3.1.8org.csv")
        )  # Original fit before improvement
        self.amjuel_data["H.2 3.1.8"] = self.read_amjuel_1d(
            os.path.join(onedrive, r"Project\Atomicrates\1D H.2 3.1.8.csv")
        )  # Low beam energy limit of 3.1.8. E=0
        self.amjuel_data["H.2 3.1.8FJ"] = self.read_amjuel_1d(
            os.path.join(onedrive, r"Project\Atomicrates\1D H.2 3.1.8FJ.csv")
        )  # Variant of unknown origin, used in EDGE2D
        self.amjuel_data["H.2 0.1T"] = self.read_amjuel_1d(
            os.path.join(onedrive, r"Project\Atomicrates\1D H.2 0.1T.csv")
        )  # Actually an elastic collision reaction - wrong.

        # Population of excited states of H
        self.amjuel_data["H.12 2.1.5"] = self.read_amjuel_2d(
            os.path.join(onedrive, r"Project\Atomicrates\H.12 2.1.5.csv")
        )  # H+/H ratio, Reiter/Sawada/Fujimoto
        self.amjuel_data["H.12 2.1.5b"] = self.read_amjuel_2d(
            os.path.join(onedrive, r"Project\Atomicrates\H.12 2.1.5b.csv")
        )  # H(2)/H ratio, Reiter/Sawada/Fujimoto NOTE NUMBERING OUT OF ORDER
        self.amjuel_data["H.12 2.1.5a"] = self.read_amjuel_2d(
            os.path.join(onedrive, r"Project\Atomicrates\H.12 2.1.5a.csv")
        )  # H(3)/H ratio, Reiter/Sawada/Fujimoto NOTE NUMBERING OUT OF ORDER
        self.amjuel_data["H.12 2.1.5c"] = self.read_amjuel_2d(
            os.path.join(onedrive, r"Project\Atomicrates\H.12 2.1.5c.csv")
        )  # H(4)/H ratio, Reiter/Sawada/Fujimoto
        self.amjuel_data["H.12 2.1.5d"] = self.read_amjuel_2d(
            os.path.join(onedrive, r"Project\Atomicrates\H.12 2.1.5d.csv")
        )  # H(5)/H ratio, Reiter/Sawada/Fujimoto
        self.amjuel_data["H.12 2.1.5e"] = self.read_amjuel_2d(
            os.path.join(onedrive, r"Project\Atomicrates\H.12 2.1.5e.csv")
        )  # H(6)/H ratio, Reiter/Sawada/Fujimoto
...
def read_case(path_case, merge_output=False):
    ...
    var_collect = [
        ...
        "S",
        "Srec",
        "Siz",  # Plasma particles and sinks
        "F",
        "Frec",
        "Fiz",
        "Fcx",
    ...
    for var in var_collect:
        try:
            result[var] = collect(
                var, path=path_case, yguards=True, info=verbosity, strict=True
            )[-1, 0, 1:-1, 0]
        except:
    ...
    try:
        result["PeSource"] = collect(
            "PeSource", path=path_case, yguards=True, info=verbosity, strict=True
        )[-1, 1:-1]
    except:
    ...
    try:
        result["NeSource"] = collect(
            "NeSource", path=path_case, yguards=True, info=verbosity, strict=True
        )[-1, 0, 1:-1]
    except:
    ...
    if guardreplaced == False:
        for var in result.keys():
            if var not in [
                ...
                "Ecx",
                "Eel",
                "Ert",  # Energy transfer between neutrals and plasma
                "Dn",
                "SNd+",
                "SPd+",
            ]:  # NVi already has its last value as the sheath flux.
    ...
    const["Pnorm"] = (
        const["Nnorm"] * const["Tnorm"] * i_charge
    )  # Converts P to Pascals. 1.602e-19 is proton charge in C
    const["Snorm"] = (
        const["Nnorm"] * const["Omega_ci"]
    )  # Normalisation for S: plasma density sink (m-3s-1)
    const["Fnorm"] = (
        i_mass * const["Nnorm"] * const["Cs0"] * const["Omega_ci"]
    )  # Plasma momentum sink normalisation (kgm-2s-1)
    const["Enorm"] = (
        i_charge * const["Nnorm"] * const["Tnorm"] * const["Omega_ci"]
    )  # Plasma energy sink normalisation
    ...
    try:
        result_dn["PeSource"] = (
            result["PeSource"] * const["Pnorm"] * const["Omega_ci"]
        )  # [Pa/s]
    except:
        pass
    ...
    result_dn["Dn"] = (
        result["Dn"] * const["rho_s0"] * const["rho_s0"] * const["Omega_ci"]
    )  # [m][m][s-1]=[m^2/s] diffusion coefficient
    ...
    result_dn["Cs"] = const["Cs0"] * np.sqrt(
        2 * result_dn["Te"] / const["Tnorm"]
    )  # Sound speed
    ...
    for i in range(2, n):
        result_dn["pos"][i] = (
            result_dn["pos"][i - 1] + 0.5 * result["dy"][i - 1] + 0.5 * result["dy"][i]
        )
...
def heat_conduction(pos, Te, kappa0=2293.8117):
    ...
    return result_pos, -2293.8117 * Te_p ** (5.0 / 2) * grad_Te
...
def thermal_convection(pos, p, vi):
    ...
    return result_pos, (5.0 / 2) * p_p * vi_p
...
def energy_flux(path, tind=-1):
    ...
    Ne = collect("Ne", path=path, tind=tind, yguards=True, info=verbosity)[
        -1, 0, 1:-1, 0
    ]
    NVi = collect("NVi", path=path, tind=tind, yguards=True, info=verbosity)[
        -1, 0, 1:-1, 0
    ]
