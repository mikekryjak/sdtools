...
def plot_profile_histories(
    ...
    for i, idx in enumerate(list_idx):
        ...
        front_start_idx = np.argmin(
            abs(Rchoice - Rchoice[Rchoice > 0.01 * Rchoice.max()][0])
        )
        ...
        ax2.plot(
            ...
            color=c,
            ls="-",
            label="Cumulative radiation integral",
            lw=2,
            alpha=1,
        )
        ...
        ax2.fill_between(
            x=[x[front_start_idx], x[front_end_idx]],
            y1=0,
            y2=1,
            color=c,
            hatch="//",
        ...
        if mode == "qpar":
            axstore[i]["ax3"].yaxis.set_major_formatter(
                mpl.ticker.StrMethodFormatter("{x:.1e}")
            )
    ...
    legend_items = [
        mpl.patches.Patch(
            ...
            hatch="\\",
            lw=2,
            label=f"{rad_threshold:.0%} Radiation",
        )
    ]
    fig.legend(
...
def get_front_widths(p, s, Rcutoff=0.7, dynamicGrid=True, use_prad=True):
    ...
    for i, _ in enumerate(s["SpolPlot"]):
        ...
        Beff = np.sqrt(
            sp.integrate.trapezoid(y=q * R, x=S)
            / sp.integrate.trapezoid(y=q * R / Btot**2, x=S)
        )
        Beff_old = sp.integrate.trapezoid(y=R * Btot, x=S) / sp.integrate.trapezoid(
            y=R, x=S
        )
        ...
        df.loc[i, "Spol_front_end"], df.loc[i, "Spol_front_start"] = get_front_ends(
            Spol
        )
        df.loc[i, "Spol_front_width"] = (
            df.loc[i, "Spol_front_start"] - df.loc[i, "Spol_front_end"]
        )
        ...
        df.loc[i, "Spar_front_width"] = (
            df.loc[i, "Spar_front_start"] - df.loc[i, "Spar_front_end"]
        )
...
def get_detachment_scalings(profiles, stores, kappa0=2500):
    ...
    front_dfs = [
        get_front_widths(profiles[x], stores[x], Rcutoff=0.5, use_prad=True)
        for x in profiles
    ]
    ...
    df["frac_gradB"] = np.array(
        [profiles[x].get_average_frac_gradB() for x in profiles]
    )
    ...
    for i, key in enumerate(stores):
        ...
        S = store[
            ...
            0
        ]  # Careful, this is the input profile not the profile the DLS ran with which is refined
        Lpar = S[-1]
        ...
        T_arb = np.linspace(
            1, 300, 100
        )  # Arbitrary temp profile for the simple DLS cooling curve integral
        Lz_arb = [Lfunc(x) for x in T_arb]
        Wradial = store["Wradials"][0]
        # Wradial_simple proportional to upstream q integral from DLS-simple
        Wradial_simple = sp.integrate.trapezoid(
            y=Btot[Xpoint:] / Bx * (Lpar - S[Xpoint:]) / (Lpar - Sx), x=S[Xpoint:]
        )
        ...
        df.loc[i, "int_qoverBsq_dt"] = np.sqrt(
            2
            * sp.integrate.trapz(
                y=qpar[Xpoint:] / (Btot[Xpoint:] ** 2 * Wradial), x=S[Xpoint:]
            )
        )
        ...
        df.loc[i, "W_Tu"] = (Wradial ** (2 / 7)) / (
            sp.integrate.trapezoid(y=qpar, x=S) ** (2 / 7)
        )
        ...
        df.loc[i, "int_TLz_dt"] = (
            np.sqrt(2 * sp.integrate.trapz(y=2500 * T**0.5 * Lz, x=T)) ** -1
        )
        ...
        df.loc[i, "C0"] = (
            7 ** (-2 / 7)
            * (2 * kappa0) ** (-3 / 14)
            * (sp.integrate.trapezoid(y=Lz * T**0.5, x=T)) ** (-0.5)
        )
        ...
        df.loc[i, "cyd_correction"] = np.sqrt(
            sp.integrate.trapz(y=qpar[Xpoint:] / (Btot[Xpoint:]) ** 2, x=S[Xpoint:])
            * store["state"].qradial
        )
...
def get_c0(store, Lfunc, kappa0=2500):
    ...
    C0 = (
        7 ** (-2 / 7)
        * (2 * kappa0) ** (-3 / 14)
        * (sp.integrate.trapezoid(y=L, x=T)) ** 0.5
    )
...
def get_band_widths(d, o, cvar, size=0.05):
    ...
    k = 5
    spar_interp = sp.interpolate.interp1d(
        crel, splot, kind="cubic", fill_value="extrapolate"
    )
    spol_interp = sp.interpolate.interp1d(
        crel, spolplot, kind="cubic", fill_value="extrapolate"
    )
...
def plot_morph_results(
    ...
    threshcalc = (
        (BxBt / BxBt_base) ** (-1)
        * (L / L_base) ** (-2 / 7)
        * (avgB_ratio / avgB_ratio_base) ** (2 / 7)
    )
    threshcalc2 = (
        (BxBt / BxBt_base) ** (-1)
        * (L / L_base) ** (-4 / 7)
    ...
    if show_47:
        ...
        label_27 = "Analytical"
    axes[1, 0].plot(
        index, threshcalc, color="darkslategrey", ls="--", label=label_27, zorder=100
    )
    ...
    axes[1, 1].plot(
        index, windowcalc, color="darkslategrey", ls="--", label=label_27, zorder=100
    )
...
def plot_profiles(
    base, stages, profiles, poloidal=True, ylims=(None, None), reverse_S=True
):
    ...
    for idx, i in enumerate(profiles):
        ...
        ax.plot(
            plotx(p),
            np.gradient(p.Btot[: p.Xpoint], p.S[: p.Xpoint]) / p.Btot[: p.Xpoint],
            zorder=100,
            alpha=1,
            color=colors[idx],
            lw=2,
    ...
    for idx, _ in enumerate(profiles):
        Lkwargs = dict(
            zorder=100, alpha=1, color=colors[idx], lw=0, marker="o", ms=10, ls="-"
        )
        BxBtkwargs = dict(
            zorder=100,
            alpha=1,
...
def plot_results(
    ...
    colors = (
        plt.rcParams["axes.prop_cycle"].by_key()["color"] if colors == None else colors
    )
    ...
    for i, id in enumerate(stores):
        ...
        ax.scatter(
            x[0], y[0], color=colors[i], marker="o", linewidths=4, s=20, zorder=10
        )
...
class plotProfiles:
    def __init__(self, profiles, colors=None, basis="Spol", side="outer", names=[]):
    ...
    def gridplot(
        ...
        if plot_all is True:
            fig, axes = plt.subplots(
                ...
                3,
                dpi=self.dpi,
                figsize=(18 * figscale, 9 * figscale),
                width_ratios=(3, 2, 2),
            )
        ...
        if profiles_only is True:
            self.plot_designs(
                eqbchoice, ax=ax, ylims=designs_ylims, xlims=designs_xlims
            )
            ylims = ax.get_ylim()
            ...
            self.plot_designs(
                eqbchoice, ax=axes[0], ylims=designs_ylims, xlims=designs_xlims
            )
            self.plot_Lc_BxBt(ax=axes[1])

        else:
            self.plot_designs(
                eqbchoice, ax=axes[0, 0], ylims=designs_ylims, xlims=designs_xlims
            )
    ...
    def plot_designs(self, eqbchoice, ax=[], xlims=(None, None), ylims=(None, None)):
        """
        ...
        for side in ["il", "ol"]:
            if type(eqbchoice) == list:
                for eqb in eqbchoice:
                    ax.plot(
                        eqb[side]["R"], eqb[side]["Z"], c="lightgrey", alpha=1, lw=3
                    )
            else:
                ax.plot(
                    eqbchoice[side]["R"],
        ...
        ax.set_title(
            "Divertor configuration", fontsize=self.titlesize, color=self.titlecolor
        )
    ...
    def plot_Lc_BxBt(self, ax=None, xlabels=False, figsize=None):
        ...
        for i, key in enumerate(profiles.keys()):
            ...
            Lkwargs = dict(
                zorder=100, alpha=1, color=colors[i], lw=0, marker="o", ms=10, ls="-"
            )
            BxBtkwargs = dict(
                zorder=100,
                alpha=1,
            ...
            if i == 0:
                Lkwargs["label"] = "Connection length factor"
            if i == 0:
                BxBtkwargs["label"] = "Flux expansion factor"
        ...
        ax.set_title(
            "Profile properties", fontsize=self.titlesize, color=self.titlecolor
        )
    ...
    def plot_summary(self, ax=None, xlabels=False, figsize=None):
        ...
        ax.plot(
            x,
            L,
            label="Connection length factor",
            marker="o",
            c="darkorange",
            **common_kwargs,
        )
    ...
    def plot_field(self, mode, ax=None, figsize=None):
        ...
        data = dict(
            ...
            Btotgrad={
                "y": lambda x: np.gradient(x.Btot, x.Spol) / x.Btot,
                "ylabel": r"$\frac{1}{B}\ \frac{dB}{dS_{\theta}}\ [m^{-1}]$",
                "title": "B field gradient",
            },
            Bpitch={
                "y": lambda x: x.Bpol / x.Btot,
        ...
        for i, key in enumerate(profiles.keys()):
            ...
            ax.set_title(
                data[mode]["title"], fontsize=self.titlesize, color=self.titlecolor
            )
...
class compare_SOLPS_DLS:
    def __init__(self, slc, out, cvar, impurity="Ar", region="outer_lower", sepadd=1):
        ...
        # Read SOLPS
        self.solps = slc.get_1d_poloidal_data(
            [
                "Btot",
                "hx",
                "vol",
    ...
    def calculate_solps(self, solps):
        ...
        solps["Qrad_per_area_cum"] = sp.integrate.cumulative_trapezoid(
            y=solps[f"R{self.impurity}"], x=solps["Spar"], initial=0
        )  # Radiation integral over volume
        solps["Qrad_per_area_cum_norm"] = (
            solps["Qrad_per_area_cum"] / solps["Qrad_per_area_cum"].max()
        )

        solps["Qrad_cum"] = sp.integrate.cumulative_trapezoid(
            y=solps[f"R{self.impurity}"] / solps["Btot"], x=solps["Spar"], initial=0
        )  # Radiation integral over volume
    ...
    def plot(
        ...
        for i, plot in enumerate(list_plots):
            if plot == "Ne":
                ...
                self.plot_radiation_integral(
                    axes[i],
                    normalise=normalise_radiation,
                    per_area=radiation_per_area,
                    plot_cz=plot_cz,
                )
            elif plot == "cz":
                self.plot_cz(axes[i])
    ...
    def plot_qpar_over_B(self, ax):
        ...
        ax.plot(
            solps["Spar"],
            abs(solps["fhex_cond"]) / solps["Btot"],
            label="SOLPS electron cond",
        )
        ax.plot(
            solps["Spar"], abs(solps["fhx_total"]) / solps["Btot"], label="SOLPS total"
        )
    ...
    def plot_radiation_integral(
        self, ax, per_area=False, logscale=False, normalise=True, plot_cz=True
    ):
        ...
        if plot_cz:
            ...
            ax2.plot(
                solps["Spar"],
                solps[f"f{self.impurity}"] * solps["ne"] ** 2,
                c="orange",
                ls="--",
                alpha=0.8,
                lw=2,
...
def test_scalings(df1, df2, param, cumulative=False):
    ...
    scalings["simple"] = (
        diff["BxBt"] ** (-1) * diff["Lc"] ** (-2 / 7) * (diff["avgB_ratio"]) ** (2 / 7)
    )
    ...
    if cumulative:
        ...
        scalings["curveclip"] = (
            diff["BxBteff"] ** (-1) * diff["W_Tu"] * diff["curveclip"]
        )
        scalings["upstream_rad"] = (
            diff["BxBteff"] ** (-1)
            * diff["W_Tu"]
            * diff["curveclip"]
            * diff["upstream_rad"]
...
class plot_comparison:
    def __init__(self, comps, studies):
        ...
        self.DLSstyles = dict(
            ...
            include_cz=dict(
                lw=0, marker="s", ms=3, markeredgewidth=1, markerfacecolor="None"
            ),
            include_cz_ne=dict(
                lw=0, marker="1", ms=6, markeredgewidth=1, markerfacecolor="None"
            ),
        )
        ...
        for i, study in enumerate(self.studies):
            elements.append(
                mpl.lines.Line2D(
                    [0],
                    [0],
                    **self.DLSstyles[study],
                    color=self.colors[study],
                    label=self.study_labels[study],
                )
            )
        elements.append(
            mpl.lines.Line2D(
                [0], [0], **self.Xpointstyle, color="deeppink", label="X-point"
            )
        )
    ...
    def plot(self, param, axes, normalise=False, **kwargs):
        ...
        for i, case in enumerate(self.cases):
            ...
            for study_no, study in enumerate(self.studies):
                ...
                ax.plot(
                    solps["Spar"], solps_data * mult, label=study, **self.SOLPSstyle
                )  # SOLPS
...
class scaling_comparison:
    def __init__(self, out1, out2, param):
        ...
        terms_check.loc["total", "cvar_ratio"] = (
            terms.loc["cvar", "test"] / terms.loc["cvar", "base"]
        ) ** (-1)

        terms["ratio"] = (terms["test"] / terms["base"]) ** (
            -1
        )  # Remember that K is positive for lower threshold
        ...
        terms_check.loc["total", "weights_pos"] = terms[terms > 0]["weights"].sum(
            axis=0
        )
