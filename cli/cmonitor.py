...
def _cmonitor_case(
    ...
    dv_noguards = (
        get_var("dx", guards=False)
        * get_var("dy", guards=False)
        * get_var("dz", guards=False)
        * get_var("J", guards=False)
    )
    ...
    if cvode:
        lratio = np.diff(nliters, prepend=nliters[1] * 0.99) / np.diff(
            nniters, prepend=nniters[1] * 0.99
        )  # Ratio of linear to nolinear iterations
    ...
    for param in res:
        if volavg:
            res[param] = (res[param] * dv_noguards[None, :, :]) / np.sum(
                dv_noguards
            )  # Volume weighted
        res[param] = np.sqrt(np.mean(res[param] ** 2, axis=(1, 2)))  # RMS
        res[param] = np.convolve(
            res[param], np.ones(1), "same"
        )  # Moving average with window of 1
    ...
    if plot is True or save is True:
        ...
        ## SNES
            ...
            log_axes[0].plot(
                snes_time[skip], snes_nonlinear_linear_ratio[skip], c="k", lw=lw
            )
            ...
            print(
                "Non-CVODE solver detected without diagnose=True under [solver], skipping diagnostic plots"
            )
        ...
        for param in res:
            ...
            big_ax.grid()
            big_ax.set_title(
                "Volume weighted RMS ddt() (normalised)", fontsize="x-small"
            )
            big_ax.legend(
                fontsize="x-small", loc="upper left", bbox_to_anchor=(1, 1), ncols=2
            )
        ...
        if logfile_plots:
            for ax in log_axes:
                ax.xaxis.set_major_locator(
                    mpl.ticker.MaxNLocator(min_n_ticks=3, nbins=4)
                )
                ax.tick_params(
                    axis="x", labelcolor="red", color="red"
                )  # Not the same time as in dataset
    ...
    if table is True:
        ...
        print("..table\n\n")
        print(
            f"it |  t[ms] |    Nsep      Nseprate |   Netarg  |   Nntarg  | Tnsol | w/s time | l/n |  nfails  | order |"
        )
        # print("~"*100)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-s",
        action="store_true",
        help="Deprecated: saving is now on by default. Do not use.",
    )
    parser.add_argument(
        "-a", action="store_true", help="Make ddt() plots non-volume averaged?"
    )
    parser.add_argument(
        "-solverdiags",
        action="store_true",
        help="Deprecated: SNES console output is now parsed by default. Do not use.",
    )
    parser.add_argument(
        "-neutrals", action="store_true", help="Alternative physics quantities"
    )
    parser.add_argument(
        "-sep",
        action="store_true",
        help="Plot profiles at sep (T) or last sol ring (F)?",
    )
    ...
    if args.s:
        print(
            "Note: -s is deprecated. Saving the figure is now on by default; please don't use -s."
        )
    if args.solverdiags:
        print(
            "Note: -solverdiags is deprecated. SNES console output is now parsed by default; please don't use -solverdiags."
        )
