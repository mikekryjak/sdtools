...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "opt", type=str, help="Modify this option in category:option format"
    )
    parser.add_argument("new_value", type=str, help="New value to set")
    parser.add_argument(
        "--preserve", action="store_true", help="Preserve results after changing input?"
    )
    ...
    for folder in os.listdir(cwd):
        if fnmatch.fnmatch(folder, fnmatch_key) and "BOUT.inp" in os.listdir(
            cwd + sep + folder
        ):
