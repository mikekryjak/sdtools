...
def make_scan(case, mode, overwrite=False):
    ...
    if intend_param != case_param:
        print(
            f"Case {case} parameter mismatch. Found: {case_param:.1E} || Case name implies: {intend_param:.1E}"
        )
        ...
        if mode == "density":
            ...
            set_opt(
                path_case,
                "p:powerflux",
                "{:.1E}".format(intend_param).replace("E+0", "e"),
            )
    ...
    for i, new_case in enumerate(new_names):
        ...
        if mode == "double_power":
            set_opt(
                path_new_case,
                "p:powerflux",
                "{:.1E}".format(scan[i] * 1e6 * 2).replace("E+0", "e"),
            )
