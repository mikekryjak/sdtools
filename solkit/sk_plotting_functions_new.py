...
class SKComp:
    ...
    def get_line_colours(self):
        ...
        if self.rundecks[0].sort_by == "density":
            ...
            sm = plt.cm.ScalarMappable(
                cmap=cmap, norm=plt.Normalize(vmin=min_dens, vmax=max_dens)
            )
            ...
            sm = plt.cm.ScalarMappable(
                cmap=cmap, norm=plt.Normalize(vmin=self.min_p, vmax=self.max_p)
            )
        ...
        for i, rundeck in enumerate(self.rundecks):
            ...
            for j in range(rundeck.num_sims):
                if rundeck.sort_by == "density":
                    self.line_colours[i][j] = sm.to_rgba(rundeck.avg_densities[j])
                elif rundeck.sort_by == "power":
                    self.line_colours[i][j] = sm.to_rgba(rundeck.input_el_powers[j])
    ...
    def build_legend(self):
        ...
        for j, run in enumerate(self.rundecks[0].runs):
            if self.rundecks[0].sort_by == "density":
                profile_label = (
                    r"$\langle n_{heavy} \rangle$"
                    + " = {:1.1f}".format(self.rundecks[0].avg_densities[j] / 1e19)
                    + r"$\times 10^{19}$m$^{-3}$"
                )
                ...
                profile_label = (
                    r"$P_{in}$"
                    + " = {:1.1f}".format(self.rundecks[0].input_el_powers[j])
                    + r"MWm$^{-2}$"
                )
                self.legend_lines.append(
                    Line2D([0], [0], color=self.line_colours[0][j])
        ...
        self.legend_lines = [
            self.legend_lines[0],
            self.legend_lines[int(len(self.legend_lines) / 2)],
            self.legend_lines[-1],
        ]
        self.legend_labels = [
            self.legend_labels[0],
            self.legend_labels[int(len(self.legend_labels) / 2)],
    ...
    def flux_rollover_plot(self, sort_by="density"):
        if sort_by == "density":
            ...
            for i, rundeck in enumerate(self.rundecks):
                ...
                self.upstream_densities[i], self.avg_densities[i] = (
                    rundeck.get_densities()
                )
                self.target_fluxes[i], self.target_temps[i] = (
                    rundeck.get_target_conditions()
                )
    ...
    def target_heat_flux_plot(self, sort_by="density", species="electrons"):
        ...
        linestyles = ["-", "-", "-", "-", "--", "--", "--", "--"]
        colours = ["red", "blue", "green", "black", "red", "blue", "green", "black"]
        labels = [
            "4MWm$^{-2}$",
            "8MWm$^{-2}$",
            "16MWm$^{-2}$",
                ...
                for j, r in enumerate(rundeck.runs):
                    if species == "electrons":
                        if r.full_fluid and (not r.intrinsic_coupling):
                            gamma[i][j] = 2.0 - 0.5 * np.log(
                                2.0
                                * np.pi
                                * (
                                    1.0
                                    + (
                                        r.data["ION_TEMPERATURE"][-1]
                                        / r.data["TEMPERATURE"][-1]
                                    )
                                )
        ...
        for i, rundeck in enumerate(self.rundecks):
            ...
            for j, r in enumerate(rundeck.runs):
                (
                    ...
                    q_ie,
                    q_cx,
                ) = r.get_energy_channels()
                E_iz[i][j] = (
                    (1e6 / el_charge) * (q_n_e + q_rad) / self.target_fluxes[i][j]
                )

            ax.plot(
                self.target_temps[i], E_iz[i], self.linestyles[i], label=self.labels[i]
            )
    ...
    def energy_discrepancy_plot(self, sort_by="density"):
        ...
        if (
            self.rundeck1.runs[0].neut_mom_eqn is False
            and self.rundeck1.runs[0].ion_temp_eqn is False
        ):
            ...
            for i, r in enumerate(self.rundeck1.runs):
                ...
                discrepancies1[i] = input_powers1[i] - (q_sh_e + q_n_e + q_rad + q_E)
            for i, r in enumerate(self.rundeck2.runs):
                (
                    input_powers2[i],
                    q_E,
                ...
                discrepancies2[i] = input_powers1[i] - (q_sh_e + q_n_e + q_rad + q_E)
            ...
            if sort_by == "power":
                ...
                ax.set_title(
                    r"$\langle n_{heavy} \rangle$"
                    + " = {:1.1f}".format(self.rundeck1.runs[0].avg_density / 1e19)
                    + r"$\times 10^{19}$m$^{-3}$",
                    loc="left",
                )
                ax.legend()
        ...
        if (
            self.rundeck1.runs[0].neut_mom_eqn is False
            and self.rundeck1.runs[0].ion_temp_eqn is True
        ):
            ...
            for i, run in enumerate(self.rundeck1.runs):
                (
                    ...
                    q_ie,
                    q_cx,
                ) = run.get_energy_channels()
                el_discrepancies1[i] = (
                    input_powers1[i] / 2 + q_ie - (q_sh_e + q_n_e + q_rad + q_E)
                )
                ion_discrepancies1[i] = (
                    input_powers1[i] / 2 + q_E + q_ion_i - (q_ie + q_rec_i + q_sh_i)
                )
            ...
            for i, run in enumerate(self.rundeck2.runs):
                ...
                el_discrepancies2[i] = (
                    input_powers2[i] / 2 + q_ie - (q_sh_e + q_n_e + q_rad + q_E)
                )
                ion_discrepancies2[i] = (
                    input_powers2[i] / 2 + q_E + q_ion_i - (q_ie + q_rec_i + q_sh_i)
                )
            ...
            if sort_by == "power":
                ...
                ax.plot(
                    input_powers1, ion_discrepancies1, "-", color="blue", label="ions"
                )
                ax.plot(input_powers2, ion_discrepancies2, "--", color="blue")
                ax.plot(
                    input_powers1,
                    el_discrepancies1 + ion_discrepancies1,
                    "-",
                    color="black",
                    label="total",
                )
                ax.plot(
                    input_powers2,
                    el_discrepancies2 + ion_discrepancies2,
                    "--",
                    color="black",
                )
                ax.set_ylabel("Discrepancy [MW$m^{-2}$]")
                ...
                _, densities2 = self.rundeck2.get_densities()
                ax.plot(
                    densities1, el_discrepancies1, "-", color="red", label="electrons"
                )
                ax.plot(densities2, el_discrepancies2, "--", color="red")
                ax.plot(densities1, ion_discrepancies1, "-", color="blue", label="ions")
        ...
        if (
            self.rundeck1.runs[0].neut_mom_eqn is True
            and self.rundeck1.runs[0].ion_temp_eqn is True
        ):
            el_discrepancies1 = np.zeros(len(self.rundeck1.runs))
            el_discrepancies2 = np.zeros(len(self.rundeck2.runs))
            ...
            for i, run in enumerate(self.rundeck1.runs):
                ...
                ion_discrepancies1[i] = (
                    ...
                    + q_E
                    + q_ion_i
                    - (q_ie + q_rec_i + q_sh_i + q_cx)
                )
                neut_discrepancies1[i] = q_cx + q_rec_i + q_recyc_n - (q_ion_i + q_sh_n)
            for i, run in enumerate(self.rundeck2.runs):
                (
                    input_powers2[i],
                    q_E,
                ...
                neut_discrepancies2[i] = q_cx + q_rec_i + q_recyc_n - (q_ion_i + q_sh_n)
            ...
            if sort_by == "power":
                ...
                ax.plot(input_powers2, neut_discrepancies2, "--", color="green")
                ax.plot(
                    input_powers1,
                    el_discrepancies1 + ion_discrepancies2,
                    "-",
                    color="black",
                    label="total plasma",
                )
        ...
        for i, rundeck in enumerate(self.rundecks):
            for j, r in enumerate(rundeck.runs):
                if j in to_plot:
                    ...
                    ax.plot(
                        r.xgrid[cells[0] : cells[1]],
                        plot_data[cells[0] : cells[1]],
                        self.linestyles[i],
                        color=self.line_colours[i][j],
                    )
    ...
    def ion_rate(self, compare_sd1d=True, sort_by="density"):
        ...
        if sort_by == "power":
            sm = plt.cm.ScalarMappable(
                cmap=cmap,
                norm=plt.Normalize(
                    vmin=self.rundeck1.runs[0].input_power,
                    vmax=self.rundeck1.runs[-1].input_power,
                ),
            )
        elif sort_by == "density":
            sm = plt.cm.ScalarMappable(
                cmap=cmap,
                norm=plt.Normalize(
                    vmin=self.rundeck1.runs[0].avg_density,
                    vmax=self.rundeck1.runs[-1].avg_density,
                ),
            )
        ...
        for r in self.rundeck1.runs:
            ...
            if sort_by == "density":
                profile_label = (
                    r"$\langle n_{heavy} \rangle$"
                    + " = {:1.1f}".format(r.avg_density / 1e19)
                    + r"$\times 10^{19}$m$^{-3}$"
                )
                c = sm.to_rgba(r.avg_density)
            elif sort_by == "power":
                profile_label = (
                    r"$P_{in}$" + " = {:1.1f}".format(r.input_power) + r"MWm$^{-2}$"
                )
                c = sm.to_rgba(r.input_power)

            if sort_by == "power":
                legend_lines.append(Line2D([0], [0], color=sm.to_rgba(r.input_power)))
            elif sort_by == "density":
        ...
        if compare_adas:
            ...
            ax.plot(adas_ion_rate[:10, 0], adas_ion_rate[:10, 1], ".-", label="ADAS")
    ...
    def ex_E_rate(self, compare_sd1d=True, sort_by="density"):
        ...
        legend_labels = []
        for r in self.rundeck1.runs:
            r.load_vars(["EX_E_RATE", "DEEX_E_RATE", "ION_E_RATE", "REC_3B_E_RATE"])
        ...
        ax.set_xlabel("$T_e$ [eV]")
        ax.set_ylabel(r"$\langle \sigma v \varepsilon \rangle^{ex}$ [eVm$^3$ s$^{-1}$]")
...
class SKRundeck:
    ...
    def load_rundeck(self, identifier):
        simulation_folders = [
            r
            for r in os.listdir(self.dir)
            if identifier in r and os.path.isdir(os.path.join(self.dir, r))
        ]
        self.runs = []
        for s in simulation_folders:
            run_numbers = sorted(
                [rn for rn in os.listdir(os.path.join(self.dir, s)) if "Run_" in rn],
                key=lambda x: int(x.strip("Run_")),
            )
    ...
    def part_source_plot(self):
        ...
        for i, r in enumerate(self.runs):
            profile_label = (
                r"$n_{u}$"
                + " = {:1.1f}".format(r.upstream_density / 1e19)
                + r"$\times 10^{19}$m$^{-3}$"
            )

            r.load_vars(["S_ION_SK", "S_ION_M"])
            S_diff = 100 * (r.data["S_ION_SK"] - r.data["S_ION_M"]) / r.data["S_ION_SK"]
            ax.plot(
                r.xgrid[::2],
                S_diff[::2],
                label=profile_label,
                color=self.line_colours[i],
            )

            ax.set_xlabel("x [m]")
            ax.set_ylabel(r"$(S_{ion}^{kin} - S_{ion}^{Max}) / S_{ion}^{kin}$ [%]")
        ...
        for i in range(self.num_sims):
            if self.sort_by == "density":
                if i == 0:
                    self.line_labels[i] = (
                        r"$\langle n_{heavy} \rangle$"
                        + " = {:1.1f}".format(self.avg_densities[i] / 1e19)
                        + r"$\times 10^{19}$m$^{-3}$"
                    )
                else:
                    self.line_labels[i] = (
                        r"{:1.1f}".format(self.avg_densities[i] / 1e19)
                        + r"$\times 10^{19}$m$^{-3}$"
                    )
            elif self.sort_by == "power":
                if i == 0:
                    self.line_labels[i] = (
                        r"$P_{in}$"
                        + " = {:1.1f}".format(self.input_el_powers[i])
                        + r"MWm$^{-2}$"
                    )
                else:
                    self.line_labels[i] = (
                        r"{:1.1f}".format(self.input_el_powers[i]) + r"MWm$^{-2}$"
                    )
    ...
    def profile(self, variable, sort_by="density", cells=(0, None)):
        ...
        for i, r in enumerate(self.runs):
            ...
            ax.plot(
                r.xgrid[cells[0] : cells[1] : 2],
                plot_data[cells[0] : cells[1] : 2],
    ...
    def temp_anisotropy_plot(self, sort_by="density", cells=(0, None)):
        ...
        for i, r in enumerate(self.runs):
            ...
            anis = (
                100
                * (r.data["TEMPERATURE_PERP"] - r.data["TEMPERATURE_PAR"])
                / r.data["TEMPERATURE"]
            )
            ax.plot(
                r.xgrid[cells[0] : cells[1]],
                anis[cells[0] : cells[1]],
                label=self.line_labels[i],
                color=self.line_colours[i],
            )
    ...
    def ionization_cost_plot(self, sort_by="density"):
        ...
        for i, r in enumerate(self.runs):
            ...
            E_iz[i] = (1e6 / el_charge) * (q_n_e + q_rad) / self.target_fluxes[i]
    ...
    def flux_rollover_plot(self):
        if self.sort_by == "density":
            ...
            fig, ax = plt.subplots(1)
            ax.plot(self.upstream_densities, self.target_fluxes, "x-", color="black")
            ax2 = ax.twinx()
            ax2.plot(self.upstream_densities, self.target_temps, "x-", color="red")
    ...
    def pressure_ratio_plot(self):
        if self.sort_by == "density":
            ...
            fig, ax = plt.subplots()
            ax.plot(self.upstream_densities, target_pressure / upstream_pressure, "x-")
            ax.set_xlabel("Upstream density [m$^{-3}$]")
    ...
    def energy_discrepancy_plot(self):
        ...
        if self.runs[0].neut_mom_eqn is False and self.runs[0].ion_temp_eqn is False:
            ...
            for i, run in enumerate(self.runs):
                ...
                discrepancies[i] = input_powers[i] - (q_sh_e + q_n_e + q_rad + q_E)
            ...
            if self.sort_by == "power":
                ...
                ax.set_title(
                    r"$\langle n_{heavy} \rangle$"
                    + " = {:1.1f}".format(self.runs[0].avg_density / 1e19)
                    + r"$\times 10^{19}$m$^{-3}$",
                    loc="left",
                )
        ...
        if self.runs[0].neut_mom_eqn is False and self.runs[0].ion_temp_eqn is True:
            ...
            for i, run in enumerate(self.runs):
                ...
                el_discrepancies[i] = (
                    input_powers[i] / 2 + q_ie - (q_sh_e + q_n_e + q_rad + q_E)
                )
                ion_discrepancies[i] = (
                    input_powers[i] / 2 + q_E + q_ion_i - (q_ie + q_rec_i + q_sh_i)
                )

            fix, ax = plt.subplots(1)
            if self.sort_by == "power":
                ax.plot(input_powers, el_discrepancies, color="red", label="electrons")
                ax.plot(input_powers, ion_discrepancies, color="blue", label="ions")
                ax.plot(
                    input_powers,
                    el_discrepancies + ion_discrepancies,
                    color="black",
                    label="total",
                )
                ax.set_ylabel("Discrepancy [MW$m^{-2}$]")
                ...
                _, densities = self.get_densities()
                ax.plot(densities, el_discrepancies, color="red", label="electrons")
                ax.plot(densities, ion_discrepancies, color="blue", label="ions")
                ax.plot(
                    densities,
                    el_discrepancies + ion_discrepancies,
                    color="black",
        ...
        if self.runs[0].neut_mom_eqn is True and self.runs[0].ion_temp_eqn is True:
            ...
            for i, run in enumerate(self.runs):
                ...
                neut_discrepancies[i] = q_cx + q_rec_i + q_recyc_n - (q_ion_i + q_sh_n)
            ...
            if self.sort_by == "power":
                ...
                ax.plot(input_powers, ion_discrepancies, color="blue", label="ions")
                ax.plot(
                    input_powers, neut_discrepancies, color="green", label="neutrals"
                )
                ax.plot(
                    input_powers,
                    el_discrepancies + ion_discrepancies,
                ...
                ax.plot(densities, el_discrepancies, color="red", label="electrons")
                ax.plot(densities, ion_discrepancies, color="blue", label="ions")
                ax.plot(densities, neut_discrepancies, color="green", label="neutrals")
                ax.plot(
                    densities,
                    el_discrepancies + ion_discrepancies,
                    color="black",
    ...
    def energy_channels_plot(self):
        ...
        if self.runs[0].neut_mom_eqn is False and self.runs[0].ion_temp_eqn is False:
            ...
            ax.legend(
                ["Radiated power", "Given to neutrals", "E field", "Sheath"],
                loc="upper left",
            )
        ...
        if self.runs[0].neut_mom_eqn is False and self.runs[0].ion_temp_eqn is True:
            ...
            if self.sort_by == "density":
                self.sort_by_density()
                ax.stackplot(
                    self.densities, q_rad, q_rec_i - q_ion_i, q_n_e, q_sh_i, q_sh_e
                )
                ax.set_xlabel("Line-averaged density [m$^{-3}$]")

            if self.sort_by == "power":
                ax.stackplot(
                    input_powers, q_rad, q_rec_i - q_ion_i, q_n_e, q_sh_i, q_sh_e
                )
            ...
            ax.legend(
                [
                    ...
                    "Given to neutrals by electrons",
                    "Sheath (ions)",
                    "Sheath (electrons)",
                ],
                loc="upper left",
        ...
        if (
            self.runs[0].neut_mom_eqn is True
            and self.runs[0].neut_temp_eqn is True
            and self.runs[0].ion_temp_eqn is True
        ):
            fig, ax = plt.subplots(1)
            ...
            ax.stackplot(
                self.densities, q_sh_i, q_sh_e, q_n_e, q_rad, q_rec_i - q_ion_i + q_cx
            )
            ax.legend(
                [
                    "expected total",
    ...
    def sort_by_density(self):
        ...
        self.runs = [
            r
            for _, r in sorted(zip(self.densities, self.runs), key=lambda pair: pair[0])
        ]
    ...
    def ion_rate(self, compare_sd1d=True, sort_by="density"):
        ...
        for i, r in enumerate(self.runs):
            ...
            ax.plot(
                T_e, sk_ion_rate, label=self.line_labels[i], color=self.line_colours[i]
            )
...
class SKRun:
    ...
    def load_avg_density(self):
        self.connection_length = self.xgrid[-1] + self.dxc[-1] / 2 + self.dxc[0] / 2
        self.plasma_density = (
            self.integrate_x(self.data["DENSITY"] * self.n_norm)
            / self.connection_length
        )
        if self.num_neutrals > 1:
            try:
                self.neutral_density = (
                    self.integrate_x(np.sum(self.data["NEUTRAL_DENS"], 1) * self.n_norm)
                    / self.connection_length
                )
            except:
                ...
                try:
                    self.neutral_density = (
                        self.integrate_x(self.data["NEUTRAL_DENS"] * self.n_norm)
                        / self.connection_length
                    )
                except:
            ...
            for variable in variables:
                if variable not in self.data.keys():
                    try:
                        ...
                        var_files = [
                            vf for vf in sorted(os.listdir(var_dir)) if variable in vf
                        ]
                        self.data[variable] = np.loadtxt(
                            os.path.join(var_dir, var_files[-1])
                        )
                    except FileNotFoundError:
                        print("WARNING: Variable " + variable + " not found in output")
    ...
    def load_dist(self):
        ...
        for l in range(self.l_max + 1):
            ...
            l_files = sorted([f for f in var_files if f[-11:-9] == str(l) + "_"])
            self.data["DIST_F"][l] = np.loadtxt(os.path.join(var_dir, l_files[-1]))
    ...
    def calc_pressure(self, interp=False):
        ...
        self.el_pressure_dyn = (
            2.0
            * el_charge
            * self.T_norm
            * self.n_norm
            * self.data["DENSITY"]
            * (self.data["FLOW_VEL_X"] ** 2)
        )

        if self.ion_temp_eqn:
            self.ion_pressure_stat = (
                el_charge
                * self.T_norm
                * self.n_norm
                * self.data["ION_TEMPERATURE"]
                * self.data["ION_DENS"]
            )
            ...
            self.ion_pressure_dyn = (ion_mass / el_mass) * self.el_pressure_dyn
        self.neut_pressure_stat = np.zeros(len(self.xgrid))
        if self.neut_temp_eqn:
            for i in range(len(self.xgrid)):
                if self.num_neutrals > 1:
                    self.neut_pressure_stat[i] = (
                        el_charge
                        * self.T_norm
                        * self.n_norm
                        * np.sum(
            ...
            for i in range(len(self.xgrid)):
                if self.num_neutrals > 1:
                    self.neut_pressure_stat[i] = (
                        ...
                        * self.n_norm
                        * (3.0 / self.T_norm)
                        * np.sum(self.data["NEUTRAL_DENS"][i])
                    )
                else:
                    self.neut_pressure_stat[i] = (
                        el_charge
                ...
                if self.num_neutrals > 1:
                    for i in range(len(self.xgrid)):
                        self.neut_pressure_dyn[i] = (
                            ...
                            * (
                                (
                                    self.data["NEUTRAL_VEL"][i]
                                    + (
                                        self.data["NEUTRAL_VEL_PERP"][i]
                                        / np.tan(np.pi * self.pitch_angle / 180.0)
                                    )
                                )
                                ** 2
                            )
        ...
        if interp is True:
            ...
            self.el_pressure_stat = self.interp_var(self.el_pressure_stat, "centre")
            self.ion_pressure_stat = self.interp_var(self.ion_pressure_stat, "centre")
            self.neut_pressure_stat = self.interp_var(self.neut_pressure_stat, "centre")
            self.el_pressure_dyn = self.interp_var(self.el_pressure_dyn, "centre")
            self.ion_pressure_dyn = self.interp_var(self.ion_pressure_dyn, "centre")
            self.neut_pressure_dyn = self.interp_var(self.neut_pressure_dyn, "centre")
    ...
    def pressure_plot(self, option=1, interp=False, cells=(0, None)):
        ...
        if option == 1:
            ...
            tot_p = (
                plasma_p + plasma_dp + self.neut_pressure_dyn + self.neut_pressure_stat
            )
            fig, ax = plt.subplots(1)
            ax.plot(
                self.xgrid[cells[0] : cells[1]],
                plasma_p[cells[0] : cells[1]],
                "-",
                color="blue",
                label="Static plasma",
            )
            ax.plot(
                self.xgrid[cells[0] : cells[1]],
                plasma_dp[cells[0] : cells[1]],
                "-",
                color="red",
                label="Dynamic plasma",
            )
            ax.plot(
                self.xgrid[cells[0] : cells[1]],
                self.neut_pressure_stat[cells[0] : cells[1]],
                "--",
                color="blue",
            ...
            ax.plot(
                self.xgrid[cells[0] : cells[1]],
                self.neut_pressure_dyn[cells[0] : cells[1]],
                "--",
                color="red",
                label="Dynamic neutral",
            )
            ax.plot(
                self.xgrid[cells[0] : cells[1]],
                tot_p[cells[0] : cells[1]],
                "-",
                color="black",
                label="Total",
            )
            ax.set_title(
                r"$\langle n_{heavy} \rangle$"
                + " = {:1.1f}".format(self.avg_density / 1e19)
                + r"$\times 10^{19}$m$^{-3}$"
            )
            ax.legend()
        if option == 2:
        ...
        if option == 3:
            ...
            ax.plot(
                self.xgrid[cells[0] : cells[1]],
                self.el_pressure_stat[cells[0] : cells[1]],
                "-",
                color="red",
                label="Electrons",
            )
            ax.plot(
                self.xgrid[cells[0] : cells[1]],
                self.el_pressure_dyn[cells[0] : cells[1]],
                "--",
                color="red",
            )
            ax.plot(
                self.xgrid[cells[0] : cells[1]],
                self.ion_pressure_stat[cells[0] : cells[1]],
                "-",
                color="blue",
                label="Ions",
            )
            ax.plot(
                self.xgrid[cells[0] : cells[1]],
                self.ion_pressure_dyn[cells[0] : cells[1]],
                "--",
                color="blue",
            )
            ax.plot(
    ...
    def get_energy_channels(self):
        ...
        if self.neut_temp_eqn:
            ...
            q_recyc_n = (
                ...
                * self.T_norm
                * T_n_recyc
                * (ion_flux / (2.0 * (self.xgrid[-1] - self.xgrid[-2])))
                * self.dxc[-1]
            )
        else:
            q_recyc_n = None
        ...
        q_E = (
            1e-6
            * (2.0 * el_charge * self.T_norm * self.n_norm / self.t_norm)
            * self.integrate_x(
                self.data["E_FIELD_X"] * self.data["DENSITY"] * self.data["FLOW_VEL_X"],
                on="centre",
            )
        )
        ...
        q_n_e = (
            1e-6
            * (self.T_norm * el_charge * self.n_norm * self.v_th / self.x_norm)
        ...
        if self.ion_temp_eqn:
            try:
                ...
                q_rec_i = q_rec_i + (3.0 / 2.0) * self.integrate_x(
                    self.data["S_REC"] * self.data["ION_TEMPERATURE"]
                )
                q_rec_i = (
                    1e-6 * q_rec_i * self.n_norm * el_charge * self.T_norm / self.t_norm
                )
            except:
                q_rec_i = 0
            if self.neut_temp_eqn:
                q_ion_i = (
                    ...
                    * self.integrate_x(
                        self.data["S_ION_SK"]
                        * self.data["NEUTRAL_VEL"]
                        * self.data["NEUTRAL_VEL"]
                    )
                )
                q_ion_i = q_ion_i + (3.0 / 2.0) * self.integrate_x(
                ...
                q_ion_i = (
                    ...
                    * (self.n_norm * el_charge * self.T_norm / self.t_norm)
                    * (3.0 / 2.0)
                    * self.integrate_x(self.data["S_ION_SK"] * neut_temp)
                )
        else:
            q_ion_i = None
            q_rec_i = None
        ...
        if self.ion_temp_eqn:
            try:
                ...
                q_ie = (
                    1e-6
                    * (3.0 / 2.0)
                    * (el_charge * self.T_norm * self.n_norm * self.v_th / self.x_norm)
                    * self.integrate_x(self.data["EI_E_RATE"] * self.data["DENSITY"])
                )
            except:
                q_ie = 0
        else:
        ...
        if self.ion_temp_eqn and self.neut_temp_eqn:
            try:
                ...
                q_cx = (
                    -1e-6
                    * (self.n_norm * el_charge * self.T_norm / self.t_norm)
                    * self.integrate_x(self.data["CX_E_RATE"])
                )
            except:
                q_cx = 0
        else:
    ...
    def q_sh_e(self):
        ...
        if self.full_fluid and (not self.intrinsic_coupling):
            gamma_e = 2.0 - 0.5 * np.log(
                2.0
                * np.pi
                * (1.0 + (T_i_boundary / T_e_boundary))
                * (el_mass / (1.0 * ion_mass))
            )
            ...
            q_sh_e = (
                (el_charge * self.n_norm * self.v_th * self.T_norm)
                * gamma_e
                * n_e_boundary
                * c_s
    ...
    def q_sh_i(self):
        ...
        self.load_vars("E_FIELD_X")
        q_sh_i += (
            (el_charge * self.T_norm * self.n_norm / self.t_norm)
            * 2.0
            * (
                self.data["E_FIELD_X"][-1]
                + 0.5 * (self.data["E_FIELD_X"][-1] - self.data["E_FIELD_X"][-2])
            )
            * self.data["DENSITY"][-1]
            * self.data["FLOW_VEL_X"][-1]
            * self.dxc[-1]
        )
    ...
    def plot_f(
        ...
        for i in range(len(cells)):
            if density_normalise:
                f.append(
                    self.data["DIST_F"][harmonic][:, cells[i]]
                    * f_norm
                    / self.data["DENSITY"][cells[i]]
                )
            else:
        ...
        for i in range(len(cells)):
            if vs == "energy":
                ...
                ax.plot(Egrid, f[i], color="red", label=label, alpha=1.0 - 0.5 * i)
                ax.set_xlabel("Energy [eV]")
                if plotmax:
                    f0_max = (
                        maxwellian(
                            self.data["TEMPERATURE"][cells[i]],
                            self.data["DENSITY"][cells[i]],
                ...
                ax.plot(self.vgrid / self.v_th, f[i], label=label, alpha=1.0 - 0.5 * i)
    ...
    def cell_power_balance(self, species, start_cell=0):
        ...
        if species == "electrons":
            ...
            heating_length = (
                self.xgrid[2 * self.n_heating - 2]
                + self.dxc[2 * self.n_heating - 2] / 2.00
                + self.dxc[0] / 2.00
            )
            for i in range(2 * self.n_heating):
                if i % 2 == 0:
                    q_in[i] = 1e6 * self.el_power * self.dxc[i] / heating_length
            q["in"] = q_in
            ...
            for i in range(self.num_x):
                if i % 2 == 0:
                    q_E[i] = (
                        (2.0 * el_charge * self.T_norm * self.n_norm / self.t_norm)
                        * self.data["E_FIELD_X"][i]
                        * self.data["DENSITY"][i]
                        * self.data["FLOW_VEL_X"][i]
                        * self.dxc[i]
                    )
            q["E"] = q_E
            ...
            self.load_vars(["EX_E_RATE", "DEEX_E_RATE", "ION_E_RATE", "REC_3B_E_RATE"])
            ...
            for i in range(self.num_x):
                if i % 2 == 0:
                    q_n[i] = (
                        ...
                        * (
                            self.data["DEEX_E_RATE"][i]
                            + self.data["REC_3B_E_RATE"][i]
                            - self.data["EX_E_RATE"][i]
                            - self.data["ION_E_RATE"][i]
                        )
                        * self.dxc[i]
                    )
            q["n"] = q_n
            ...
            fig, ax = plt.subplots(1)
            ax.plot(
                self.xgrid[start_cell::2], 1e-6 * q_in[start_cell::2], label="$q_{in}$"
            )
            ax.plot(
                self.xgrid[start_cell::2],
                1e-6 * (q_L[start_cell::2] - q_R[start_cell::2]),
                label="$q_L - q_R$",
            )
            ax.plot(self.xgrid[start_cell::2], 1e-6 * q_n[start_cell::2], label="$q_n$")
            ax.plot(
                self.xgrid[start_cell::2], -1e-6 * q_E[start_cell::2], label="$q_E$"
            )
            if self.ion_temp_eqn:
                q_tot = (q_L - q_R) + q_n - q_E + q_in + q_ie
                ax.plot(
                    self.xgrid[start_cell::2],
                    1e-6 * q_ie[start_cell::2],
                    label="$q_{ie}$",
                )
            else:
            ...
            for i in range(2 * self.n_heating):
                if i % 2 == 0:
                    q_in[i] = 1e6 * self.ion_power * self.dxc[i] / heating_length
            q["in"] = q_in
            ...
            for i in range(self.num_x):
                if i % 2 == 0:
                    ...
                    q_rec = (
                        ...
                        + (3.0 / 2.0)
                        * self.data["S_REC"][i]
                        * self.data["ION_TEMPERATURE"][i]
                        * self.dxc[i]
                    )
                    q_rec *= self.n_norm * el_charge * self.T_norm / self.t_norm
                    q_ion = (
                        (ion_mass / el_mass)
                        * self.data["S_ION_SK"][i]
                        * neut_vel[i]
                        * neut_vel[i]
                        * self.dxc[i]
                    )
                    ...
                    q_ion *= self.n_norm * el_charge * self.T_norm / self.t_norm
                    q_n[i] = q_ion - q_rec
            q["n"] = q_n
            ...
            ax.plot(self.xgrid[start_cell::2], 1e-6 * q_n[start_cell::2], label="$q_n$")
            ax.plot(self.xgrid[start_cell::2], 1e-6 * q_E[start_cell::2], label="$q_E$")
            ax.plot(
                self.xgrid[start_cell::2], -1e-6 * q_ie[start_cell::2], label="$q_{ie}$"
            )
            if self.neut_temp_eqn and self.neut_mom_eqn:
                ax.plot(
                    self.xgrid[start_cell::2],
                    1e-6 * q_cx[start_cell::2],
                    label="$q_{cx}$",
                )
    ...
    def heat_fluxes(self):
        ...
        if self.full_fluid and (not self.intrinsic_coupling):
            gamma_e = 2.0 - 0.5 * np.log(
                ...
                * (
                    1.0
                    + (self.data["ION_TEMPERATURE"][-1] / self.data["TEMPERATURE"][-1])
                )
                * (el_mass / (1.0 * ion_mass))
            )
        else:
        ...
        q_conv[-1] = (
            2.5 * el_charge * self.T_norm * self.data["TEMPERATURE"][-1] * ion_flux
        )
    ...
    def energy_flux_v_plot(self, intrinsic=True, cell=-1, vs="velocity"):
        ...
        if intrinsic is False:
            ...
            if len(self.data["DIST_F"]) >= 1:
                ...
                for i, v in enumerate(self.vgrid):
                    ...
                    U_e = el_mass * 4.0 * np.pi * f0[i] * (v**4) - (
                        el_mass * n_e * v_u * v_u
                    )
                    P_11 = (U_e / 3) + (
                        (2 / 15) * (el_mass * 4.0 * np.pi * f2[i] * (v**4))
                    )
                    c = u_e * P_11
                    q_v[i] = a - b - c
        ...
        if vs == "velocity":
            v_th = np.sqrt(
                el_charge * self.T_norm * self.data["TEMPERATURE"][cell] / el_mass
            )
            ax.plot(
                self.vgrid / v_th,
                q_v / np.max(q_v),
            ...
            ax.plot(E, q_v / np.max(q_v), "-", color="red", label=integrand_label)
            ax.plot(E, q_tot / np.max(q_tot), "--", color="red", label="Integral")
            ax.set_xlabel("$E [eV]$")
    ...
    def sankey_diagram(self, separate=True):
        ...
        if self.ion_temp_eqn is False and self.neut_mom_eqn is False:
            ...
            sankey.add(
                flows=[input_power, -q_E, -q_sh_e, -q_n_e, -q_rad],
                labels=["Input", "$q_E$", "$q_{sh,e}$", "$q_{n,e}$", "$q_{rad}$"],
                orientations=[0, 1, 0, -1, -1],
                pathlengths=[0.25, 0.25, 0.25, 0.25, 0.25],
            )  # Arguments to matplotlib.patches.PathPatch
            diagrams = sankey.finish()
            ax.set_title(
                "Electron flux channels [MWm$^{-2}$]."
                + "\n"
                + r"$\langle n_{heavy} \rangle$"
                + " = {:1.1f}".format(self.avg_density / 1e19)
                + r"$\times 10^{19}$m$^{-3}$"
                + "\nDiscrepancy = {0:.2f} MW/m^2".format(
                    input_power - (q_sh_e + q_n_e + q_rad + q_E)
                ),
                size=10,
            )
        # Diffusive neutrals, with T_i equation
        if self.ion_temp_eqn is True and self.neut_mom_eqn is False:
            if separate is True:
                ...
                ax[0].set_yticks([])
                ax[0].axis("off")
                sankey = Sankey(ax=ax[0], scale=0.1, head_angle=150, format="%.2f")
                sankey.add(
                    flows=[input_power / 2, q_ie, -q_E, -q_sh_e, -q_n_e, -q_rad],
                    labels=[
                        "Input",
                        "$q_{ie}$",
                        "$q_E$",
                ...
                ax[1].set_yticks([])
                ax[1].axis("off")
                ion_sankey = Sankey(ax=ax[1], scale=0.1, head_angle=150, format="%.2f")
                ion_sankey.add(
                    color="black",
                    flows=[input_power / 2, q_E, -q_ie, q_ion_i - q_rec_i, -q_sh_i],
                    labels=[
                        "Input",
                        "$q_{E}$",
                        "$q_{ie}$",
        ...
        if (
            self.ion_temp_eqn is True
            and self.neut_mom_eqn is True
            and self.neut_temp_eqn is True
        ):
            if self.periodic is True:
                # Electrons
                fig = plt.figure()
                ax = fig.add_subplot(
                    1, 1, 1, xticks=[], yticks=[], title="Electrons [MWm$^{-2}$]"
                )
                sankey = Sankey(ax=ax, scale=0.1, head_angle=150, format="%.2f")
                sankey.add(
                    flows=[input_power / 2, q_ie, -q_n_e, -q_rad],
                    labels=["Input", "$q_{ie}$", "$q_{n,e}$", "$q_{rad}$"],
                ...
                ion_sankey = Sankey(ax=ion_ax, scale=0.1, head_angle=150, format="%.2f")
                ion_sankey.add(
                    color="black",
                    flows=[input_power / 2, q_ion_i, -q_ie, -q_rec_i, -q_cx],
                    labels=[
                        "Input",
                        "$q_{ion,i}$",
                        "$q_{ie}$",
                ...
                neut_sankey = Sankey(
                    ax=neut_ax, scale=0.1, offset=0.2, head_angle=150, format="%.2f"
                )
                neut_sankey.add(
                    color="black",
                    flows=[q_cx, q_rec_i, -q_ion_i],
                    labels=["$q_{cx}$", "$q_{rec,i}$", "$q_{ion,i}$"],
                    orientations=[0, 1, 1],
                )  # Arguments to matplotlib.patches.PathPatch
                neut_diagrams = neut_sankey.finish()
                neut_ax.set_title(
                    "Neutral flux channels [MWm$^{-2}$]. "
                    + "Discrepancy = {0:.2f}".format(q_cx + q_rec_i - (q_ion_i))
                )
                return
            else:
                pass
            ...
            if separate is True:
                ...
                sankey = Sankey(
                    ax=ax[0, 0], scale=0.1, offset=0.2, head_angle=150, format="%.2f"
                )
                sankey.add(
                    flows=[input_power / 2, q_ie, -q_sh_e, -q_n_e, -q_rad, -q_E],
                    labels=[
                        "input",
                        "i-e transfer",
                        "sheath",
                    ...
                    pathlengths=[0.25, 0.25, 0.25, 0.75, 0.25, 0.25],
                )  # Arguments to matplotlib.patches.PathPatch
                diagrams = sankey.finish()
                ax[0, 0].set_title(
                    "Electron flux channels [MWm$^{-2}$].\n"
                ...
                ion_sankey.add(
                    ...
                    flows=[
                        ...
                        -q_ie,
                        -q_sh_i,
                        -q_cx + q_ion_i - q_rec_i,
                    ],
                    labels=["input", "E-field", "i-e transfer", "sheath", "CX, etc"],
                ...
                neut_sankey.add(
                    color="black",
                    flows=[q_recyc_n, q_cx + q_rec_i - q_ion_i, -q_sh_n],
    ...
    def reconstruct_f_plot(
        self,
        cart_transform=False,
        cell=-1,
        num_contours=10,
        loglin=True,
        max_vth=10,
        ...
        for l in range(self.l_max + 1):
            for i, theta in enumerate(theta_grid):
                f_pol[:, i] += self.data["DIST_F"][l][:, cell] * legendre_coeff(
                    l, np.cos(theta)
                )
        ...
        if cart_transform:
            ...
            vgrid_par = (
                np.linspace(-self.vgrid[-1], self.vgrid[-1], 2 * cart_num_v) / v_th
            )
            ...
            for i in range(len(vgrid_par)):
                for j in range(len(vgrid_perp)):
                    ...
                    f_cart[i, j] = interp_f(f_pol, v, th, vgrid_norm, theta_grid)
            ...
            cs = ax.contourf(indep_gridx, indep_gridy, f_cart_plot, levels=num_contours)
            ...
            if loglin:
                ...
                if min_f <= 0.0:
                    min_f = 1e-30
                lev_exp = np.arange(
                    np.floor(np.log10(min_f) - 1),
                    np.ceil(np.log10(f_pol_plot.max()) + 1),
                )
                levs = np.power(10, lev_exp)
                cs = ax.contourf(
                    theta_grid, indep_grid, f_pol_plot, levs, norm=colors.LogNorm()
                )
            else:
                cs = ax.contourf(
                    theta_grid, indep_grid, f_pol_plot, levels=num_contours
                )
    ...
    def ion_rate(self):
        ...
        if compare_sd1d:
            sd1d_ion_rate = SD1D_ion_rate()
            ax.plot(sd1d_ion_rate[:, 0], sd1d_ion_rate[:, 1], "--", label="SD1D")
    ...
    def ion_ex_E_rate(self, compare_sd1d=True):
        ...
        q_n = (self.T_norm * self.n_norm * self.v_th / self.x_norm) * (
            self.data["EX_E_RATE"]
            + self.data["ION_E_RATE"]
            - self.data["DEEX_E_RATE"]
            - self.data["REC_3B_E_RATE"]
        )
        ...
        ax.plot(T_e[::2], sigma_v_eps_exonly[::2], "--", label="SOL-KiT (ex+deex only)")
        ax.set_xlabel("$T_e$ [eV]")
        ax.set_ylabel(r"$\langle \sigma v \varepsilon \rangle$ [eV m$^3$ s$^{-1}$]")
        ax.legend()
        ax.grid()
        plt.yscale("log")
    ...
    def ionisation_degree_plot(self, compare_amjuel=True):
        ...
        for i in range(len(T_e)):
            amjuel_iz_degree[i] = 100.0 / (
                1
                + (
                    amjuel_rate("recombination", 0, n_e[i], T_e[i])
                    / amjuel_rate("ionisation", 0, n_e[i], T_e[i])
                )
            )

        sd1d_iz_degree = np.zeros(len(T_e))
        for i in range(len(T_e)):
            sd1d_iz_degree[i] = 100.0 / (
                1
                + (
                    amjuel_rate("recombination", 0, 1e14, T_e[i])
                    / amjuel_rate("ionisation", 0, 1e14, T_e[i])
                )
            )
        ...
        ax.set_xlabel("$T_e$ [eV]")
        ax.set_ylabel(r"Ionisation degree (%)")
        ax.legend()
        ax.grid()
        # plt.yscale('log')
        # plt.xscale('log')

    def neutral_distribution(
        self, cells=None, degeneracy_norm=False, energy_axis=False
    ):
    ...
    def q_n_comparison_plot(self, compare_sd1d=False, compare_amjuel=True):
        ...
        if compare_amjuel:
            ...
            for i in range(self.num_x):
                q_n_amjuel_ionex = (
                    amjuel_rate("ionisation", 2, n_e[i], T_e[i]) * n_e[i] * n_n[i]
                )
                q_n_amjuel_rec = (
                    amjuel_rate("recombination", 2, n_e[i], T_e[i]) * n_e[i] * n_e[i]
                )
                q_n_amjuel_rec -= (
                    13.6
                    * amjuel_rate("recombination", 0, n_e[i], T_e[i])
                ...
                q_n_amjuel[i] = 1e-6 * el_charge * (q_n_amjuel_ionex - q_n_amjuel_rec)
    ...
    def part_source_comparison_plot(self, compare_sd1d=True, compare_amjuel=True):
        ...
        self.load_vars(["S_ION_SK", "S_REC"])
        s_sk = (self.data["S_ION_SK"] - self.data["S_REC"]) * self.n_norm / self.t_norm
        ...
        if compare_sd1d:
            ...
            for i in range(self.num_x):
                s_sd1d[i] = amjuel_rate("ionisation", 0, 1e14, T_e[i]) * n_e[i] * n_n[i]
                s_sd1d[i] -= (
                    amjuel_rate("recombination", 0, 1e14, T_e[i]) * n_e[i] * n_e[i]
                )
        ...
        if compare_amjuel:
            ...
            for i in range(self.num_x):
                s_amjuel[i] = (
                    amjuel_rate("ionisation", 0, n_e[i], T_e[i]) * n_e[i] * n_n[i]
                )
                s_amjuel[i] -= (
                    amjuel_rate("recombination", 0, n_e[i], T_e[i]) * n_e[i] * n_e[i]
                )
...
class SKTRun(SKRun):
    ...
    def get_timestamps(self):
        ...
        for run_num in self.run_nums:
            gridfile = os.path.join(self.top_dir, run_num, "INPUT", "GRID_INPUT.txt")
        ...
        for i, run_num in enumerate(self.run_nums):
            saved_files = os.listdir(
                os.path.join(self.top_dir, run_num, "OUTPUT", "DENSITY")
            )
            saved_timesteps = sorted(
                [int(f[8:-4]) for f in saved_files if "DENSITY_" in f]
            )
            if i == 0:
                ...
                timestamps = timestamps + [
                    (t * self.dts[i] * self.t_norm) + prev_ts for t in saved_timesteps
                ]
                ...
                if variable == "DIST_F":
                    try:
                        ...
                        for run_num in self.run_nums:
                            ...
                            dist_files = dist_files + [
                                os.path.join(var_dir, vf)
                                for vf in os.listdir(var_dir)
                                if os.path.basename(vf).startswith("F_L  ")
                            ]
                        ...
                        for l in range(self.l_max + 1):
                            l_files = sorted(
                                ...
                                key=lambda x: (
                                    float((x.split(os.sep))[-4].strip("Run_")),
                                    int(os.path.basename(x)[-9:-4]),
                                ),
                            )
                            for i, l_file in enumerate(l_files):
                                self.tdata["DIST_F"][i, l, :, :] = np.loadtxt(l_file)
                    ...
                    try:
                        ...
                        if variable == "SHEATH_DATA":
                            self.tdata[variable] = np.zeros([len(self.timestamps), 5])
                        else:
                        ...
                        for i, vf in enumerate(var_files):
                            if variable == "NEUTRAL_DENS":
                                self.tdata[variable][i, :] = np.sum(np.loadtxt(vf), 1)
                            else:
    ...
    def animate(self, variable, interval=100):
        ...
        ax.set_ylim([np.min(self.tdata[variable]), np.max(self.tdata[variable])])
        ax.set_xlabel("x [m]")
    ...
    def get_target_density(self, i):
        ...
        return (
            self.tdata["DENSITY"][i, -1]
            * self.tdata["DENSITY"][i, -1]
            / self.tdata["DENSITY"][i, -2]
        )
    ...
    def get_target_flux(self, i):
        ...
        flux = n_e_boundary * sound_speed(T_e_boundary, T_i_boundary) * self.v_th
        return flux
    ...
    def q_sh_e(self, i):
        ...
        if self.ion_temp_eqn:
            ...
            T_i_boundary = T_e_boundary
        c_s = (
            sound_speed(T_e_boundary / self.T_norm, T_i_boundary / self.T_norm)
            * self.v_th
        )
        n_e_boundary = (
            self.tdata["DENSITY"][i, -1]
        ...
        q_sh_e = (
            el_charge * gamma_e * n_e_boundary * c_s * self.tdata["TEMPERATURE"][i, -1]
        )
    ...
    def burnthrough_plot(self):
        ...
        for i, t in enumerate(self.timestamps):
            avg_dens_plasma = (
                np.sum(self.tdata["DENSITY"][i, :] * self.dxc) / self.connection_length
            )
            avg_dens_neuts = (
                np.sum(self.tdata["NEUTRAL_DENS"][i, :] * self.dxc)
                / self.connection_length
            )
            ionization_degree[i] = 100.0 * (1.0 - avg_dens_neuts / avg_dens_plasma)
...
class SKTRundeck(SKRundeck):
    ...
    def animate(self, variable, interval=5):
        ...
        def update_lines(i):
            ...
            for j in range(self.num_sims):
                ts_counts[j] = min(
                    bisect.bisect_left(self.truns[j].timestamps, cur_ts),
                    len(self.truns[j].timestamps) - 1,
                )
                lines[j].set_data(
    ...
    def timeseries(self, variable, cell=-1):
        ...
        ax.set_xlabel("Time [$\mu$s]")
        ax.set_ylabel(self.truns[0].get_var_label(variable))
        ax.set_title(variable + " at x={:.2f}m".format(self.truns[0].xgrid[cell]))
    ...
    def burnthrough_plot(self):
        ...
        for i, trun in enumerate(self.truns):
            ...
            for j, t in enumerate(trun.timestamps):
                avg_dens_plasma = (
                    np.sum(trun.tdata["DENSITY"][j, :] * trun.dxc)
                    / trun.connection_length
                )
                avg_dens_neuts = (
                    np.sum(trun.tdata["NEUTRAL_DENS"][j, :] * trun.dxc)
                    / trun.connection_length
                )
    ...
    def flux_rollover_plot(self, interval=0, anim_dt=1, anim_max_ts=None):
        ...
        for i, trun in enumerate(self.truns):
            ...
            for j in range(len(trun.timestamps)):
                T_t[i][j] = 0.5 * (
                    trun.tdata["TEMPERATURE"][j, -1]
                    + trun.tdata["ION_TEMPERATURE"][j, -1]
                )
        ...
        (Gamma_t_line,) = ax.plot(
            self.avg_densities,
            [Gamma_t[i][0] for i in range(self.num_sims)],
            "x-",
            color="black",
        )
        ax2 = ax.twinx()
        (T_t_line,) = ax2.plot(
            self.avg_densities,
            [T_t[i][0] for i in range(self.num_sims)],
            "x-",
            color="red",
        )
        ax.set_ylabel("Target ion flux [m$^{-2}$s$^{-1}$]")
        ...
        ax2.tick_params(axis="y", colors="red")
        ax.set_xlabel(r"Line-averaged density [m$^{-3}$]")
        ax2.set_ylim([np.min(T_t[~np.isnan(T_t)]), np.max(T_t[~np.isnan(T_t)])])
        ax.set_ylim(
            [np.min(Gamma_t[~np.isnan(Gamma_t)]), np.max(Gamma_t[~np.isnan(Gamma_t)])]
        )
        ...
        def update_lines(i):
            cur_ts = anim_dt * i
            ax.set_title("{:.1f} $\mu$s".format(1e6 * cur_ts))
            T_t_line.set_data(self.avg_densities, T_t[:, i])
            Gamma_t_line.set_data(self.avg_densities, Gamma_t[:, i])

        anim = animation.FuncAnimation(
            fig,
            update_lines,
            frames=len(anim_timestamps),
            interval=interval,
            blit=False,
            repeat=True,
        )
    ...
    def q_sh_e_plot(self, interval=0, anim_dt=1, anim_max_ts=None):
        ...
        for i, trun in enumerate(self.truns):
            trun.load_tvars(
                ["TEMPERATURE", "ION_TEMPERATURE", "DENSITY", "SHEATH_DATA"]
            )
        ...
        # Set up figure
        fig, ax = plt.subplots(1)
        (q_sh_line,) = ax.plot(self.avg_densities, q_sh[:, 0], "x-", color="black")
        ax.set_ylabel("Target heat flux [MWm$^{-2}$]")
        ax.set_xlabel(r"Line-averaged density [m$^{-3}$]")
        ax.set_ylim([np.min(q_sh[~np.isnan(q_sh)]), np.max(q_sh[~np.isnan(q_sh)])])

        # Animate
        def update_lines(i):
            cur_ts = anim_dt * i
            ax.set_title("{:.1f} $\mu$s".format(1e6 * cur_ts))
            q_sh_line.set_data(self.avg_densities, q_sh[:, i])
...
class SKTComp(SKComp):
    ...
    def timeseries(self, variable, cell=-1):
        ...
        for j, trundeck in enumerate(self.trundecks):
            for i, trun in enumerate(trundeck.truns):
                ax.plot(
                    trun.timestamps * 1e6,
                    all_tseries_comp[j][i],
                    linestyle=self.linestyles[j],
                    color=self.line_colours[j][i],
                )
        ...
        def update_lines(i):
            ...
            for k, trundeck in enumerate(self.trundecks):
                for j in range(trundeck.num_sims):
                    ...
                    lines[k][j].set_data(
                        trundeck.truns[j].xgrid,
                        trundeck.truns[j].tdata[variable][ts_counts[k][j], :],
                    )
            return (lines,)
        ...
        for j, trundeck in enumerate(self.trundecks):
            ...
            for i, trun in enumerate(trundeck.truns):
                ...
                for k, t in enumerate(trun.timestamps):
                    avg_dens_plasma = (
                        np.sum(trun.tdata["DENSITY"][k, :] * trun.dxc)
                        / trun.connection_length
                    )
                    avg_dens_neuts = (
                        np.sum(trun.tdata["NEUTRAL_DENS"][k, :] * trun.dxc)
                        / trun.connection_length
                    )
    ...
    def flux_rollover_plot(self, interval=0, anim_dt=1, anim_max_ts=None):
        ...
        # Get limits
        min_T_t = np.min(
            [np.min(T_t[k][~np.isnan(T_t[k])]) for k in range(len(self.trundecks))]
        )
        max_T_t = np.max(
            [np.max(T_t[k][~np.isnan(T_t[k])]) for k in range(len(self.trundecks))]
        )
        min_Gamma_t = np.min(
            [
                np.min(Gamma_t[k][~np.isnan(Gamma_t[k])])
                for k in range(len(self.trundecks))
            ]
        )
        max_Gamma_t = np.max(
            [
                np.max(Gamma_t[k][~np.isnan(Gamma_t[k])])
                for k in range(len(self.trundecks))
            ]
        )
        ...
        for k, trundeck in enumerate(self.trundecks):
            (Gamma_t_lines[k],) = ax.plot(
                trundeck.avg_densities,
                Gamma_t[k][:, 0],
                marker="x",
                linestyle=self.linestyles[k],
                color="black",
                label=self.labels[k],
            )
        ...
        def update_lines(i):
            ...
            ax.set_title("{:.1f} $\mu$s".format(1e6 * cur_ts))
            for k, trundeck in enumerate(self.trundecks):
                T_t_lines[k].set_data(trundeck.avg_densities, T_t[k][:, i])
                Gamma_t_lines[k].set_data(trundeck.avg_densities, Gamma_t[k][:, i])
    ...
    def q_sh_e_plot(self, interval=0, anim_dt=1, anim_max_ts=None):
        ...
        # Get limits
        min_q_sh = np.min(
            [np.min(q_sh[k][~np.isnan(q_sh[k])]) for k in range(len(self.trundecks))]
        )
        max_q_sh = np.max(
            [np.max(q_sh[k][~np.isnan(q_sh[k])]) for k in range(len(self.trundecks))]
        )
        ...
        def update_lines(i):
            ...
            ax.set_title("{:.1f} $\mu$s".format(1e6 * cur_ts))
            for k, trundeck in enumerate(self.trundecks):
                q_sh_lines[k].set_data(trundeck.avg_densities, q_sh[k][:, i])
...
def SD1D_ion_rate():
    ion_rate = np.array(
        [
            [1.0395725771549431, 2.2303299924276372e-20],
...
def maxwellian(T, n, vgrid=None):
    ...
    if vgrid is None:
        velocities = np.arange(0.00001, 10, 1.0 / 1000.0)
    else:
        velocities = vgrid
