...
class SKComp:
    ...
    def flux_rollover_plot(self, sort_by="density"):
        if sort_by == "density":
            ...
            self.upstream_densities1, self.avg_densities1 = (
                self.rundeck1.get_densities()
            )
            self.upstream_densities2, self.avg_densities2 = (
                self.rundeck2.get_densities()
            )
            self.target_fluxes1, self.target_temps1 = (
                self.rundeck1.get_target_conditions()
            )
            self.target_fluxes2, self.target_temps2 = (
                self.rundeck2.get_target_conditions()
            )
    ...
    def profile(self, variable, sort_by="density", cells=(0, None)):
        ...
        sm = plt.cm.ScalarMappable(
            ...
            norm=plt.Normalize(
                vmin=min(
                    self.rundeck1.runs[0].avg_density, self.rundeck2.runs[0].avg_density
                ),
                vmax=1.25
                * max(
                    self.rundeck1.runs[-1].avg_density,
                    self.rundeck2.runs[-1].avg_density,
                ),
            ),
        )
        ...
        for r in self.rundeck1.runs:
            ...
            ax.plot(
                ...
                label=profile_label,
                color=sm.to_rgba(r.avg_density),
            )
    ...
    def profile(self, variable, sort_by="density", cells=(0, None)):
        ...
        for r in self.runs:
            ...
            ax.plot(
                ...
                plot_data[cells[0] : cells[1]],
                label=profile_label,
                color=sm.to_rgba(r.upstream_density),
            )
        ax.legend()
        ax.set_ylabel(y_label)
        ax.set_xlabel("x [m]")
...
class SKRun:
    ...
    def calc_pressure(self, interp=False):
        ...
        if self.ion_temp_eqn:
            ...
            self.ion_pressure_dyn = 2.0 * (ion_mass / el_mass) * self.el_pressure_dyn
    ...
    def q_sh_e(self):
        ...
        gamma_e = 2.0 - 0.5 * np.log(2.0 * np.pi * (2.0) * (el_mass / (1.0 * ion_mass)))

        flux_from_T_eqn = (el_charge * self.T_norm * self.v_th) * (
            ((2 / 3) * self.data["TEMPERATURE"][-1] * c_s)
            + (
                (2 / (3 * self.data["DENSITY"][-1]))
                * (gamma_e - 2.5)
                * T_e_boundary
                * c_s
                * n_e_boundary
        ...
        q_sh_e = (
            (self.n_norm * self.data["DENSITY"][-1])
            * (
                (3 / 2)
                + (
                    ...
                    * (
                        ...
                        / np.sqrt(self.data["TEMPERATURE"][-1])
                    )
                )
            )
            * flux_from_T_eqn
    ...
    def q_sh_i(self):
        ...
        q_sh_i = (
            (el_charge * self.T_norm * self.v_th * self.n_norm)
    ...
    def heat_fluxes(self):
        ...
        if self.full_fluid and (not self.intrinsic_coupling):
            gamma_e = 2.0 - 0.5 * np.log(
                2.0
                * np.pi
                * (1.0 + (self.data["TEMPERATURE"][-1] / self.data["TEMPERATURE"][-1]))
