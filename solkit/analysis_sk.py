...
class SKDeck:
    def __init__(self, path_deck, key="", keys=[], load_all=True):
        ...
        if load_all:
            ...
            for i, casename in enumerate(self.casenames):
                self.cases[casename].sk = self.read_sk(
                    self.casepaths[i], self.timesteps[i]
                )
    ...
    def read_sk(self, path_solkit, timestep, verbose=False):
        ...
        if mode == "Windows":
            ...
            sk["Te"] = (
                np.loadtxt(
                    path_case_sk + r"\TEMPERATURE\TEMPERATURE_{}.txt".format(timestep)
                )
                * T_norm
            )  # eV
            sk["Ne"] = (
                np.loadtxt(path_case_sk + r"\DENSITY\DENSITY_{}.txt".format(timestep))
                * n_norm
            )  # in m-3
            sk["Nn"] = (
                np.loadtxt(
                    path_case_sk + r"\NEUTRAL_DENS\NEUTRAL_DENS_{}.txt".format(timestep)
                )
                * n_norm
            )  # in m-3, normalised by n_norm
            sk["x_grid"] = (
                np.loadtxt(path_case_sk + r"\GRIDS\X_GRID.txt") * x_norm
            )  # in m, normalised by x_norm
            sk["heat_flow_x"] = (
                np.loadtxt(
                    path_case_sk + r"\HEAT_FLOW_X\HEAT_FLOW_X_{}.txt".format(timestep)
                )
                * q_norm
            )  # electron conduction heat flux
            sk["Vi"] = (
                np.loadtxt(path_case_sk + r"\ION_VEL\ION_VEL_{}.txt".format(timestep))
                * v_t
            )
        ...
        params["S_ION_M"]["name"] = (
            "S_ION_M"  # Maxwellian rates. S_ION_SK is kinetic rates
        )
        ...
        for param in params.keys():
            try:
                sk_raw[param] = np.loadtxt(
                    path_case_sk
                    + r"\{}\{}_{}.txt".format(
                        params[param]["name"], params[param]["name"], timestep
                    )
                )
                ...
                if verbose:
                    print(
                        ">> Could not read {} ({})".format(param, params[param]["name"])
                    )
        ...
        # print(sk["x_grid"])
        sk["heating_length"] = (
            sk["x_grid"][int(2 * sk["N_HEATING"] - 2)]
            + dxc[int(2 * sk["N_HEATING"] - 2)] / 2
            + dxc[0] / 2
        )
        ...
        if "Ti" not in list_missing:
            ...
            sk["Cs"] = np.sqrt(
                1 * Z * el_charge * (sk["Ti"] + sk["Te"]) / i_mass
            )  # Gamma is 1 not 5/3 because in 1D you have 1 DOF.
        # k_b needs keV
        ...
        sk["Te"] = np.insert(sk["Te"], len(sk["Te"]), sk["Te"][-1])

        if "Ti" not in list_missing:
            sk["Ti"] = np.insert(sk["Ti"], len(sk["Ti"]), sk["Ti"][-1])

        # Sound speed: same as temperature
        sk["Cs"] = np.insert(sk["Cs"], len(sk["Cs"]), sk["Cs"][-1])

        # Mach number: set to 1
        sk["M"] = np.insert(sk["M"], len(sk["M"]), 1)

        # Velocity: based on M=1
        sk["Vi"] = np.insert(sk["Vi"], len(sk["Vi"]), sk["Cs"][-1])
        ...
        sk["Ne"] = np.insert(
            sk["Ne"], len(sk["Ne"]), sk["Ne"][-1] * (sk["Ne"][-1] / sk["Ne"][-2])
        )

        if "Ni" not in list_missing:
            sk["Ni"] = np.insert(
                sk["Ni"], len(sk["Ni"]), sk["Ni"][-1] * (sk["Ni"][-1] / sk["Ni"][-2])
            )
        ...
        sk["Nn"] = np.insert(
            sk["Nn"], len(sk["Nn"]), sk["Nn"][-1] * (sk["Nn"][-1] / sk["Nn"][-2])
        )
        ...
        if "Ti" not in list_missing:
            ...
            sk["T_mean"] = [
                np.mean([sk["Te"][x], sk["Ti"][x]]) for x in range(len(sk["Te"]))
            ]
