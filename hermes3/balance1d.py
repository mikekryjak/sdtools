...
class Balance1D:
    def __init__(
        ...
        self.calculate_sheath_fluxes(
            method=sheath_flux_method
        )  # Now have .sheath dict with properties
        ...
        for param in [
            ...
            if param in ds:
                ...
                if param == f"R{self.impurity_name}":
                    hbal[param] = -1 * abs(
                        hbal[param]
                    )  # Correct inconsistency in sign convention
            else:
                if self.verbose:
                    print(f"Warning! {param} not found, results may be incorrect")
    ...
    def get_tallies(self):
        ...
        pbalerror = {}
        pbalerror["imbalance"] = (
            pbalsum["sources_sum"] - pbal["Sd_target_recycle"]
        ) / pbalsum["sources_sum"]
        ...
        hbalsum["R_imp_sum"] = (
            hbal[f"R{self.impurity_name}"]
            if f"R{self.impurity_name}" in ds
            else np.zeros_like(hbal["Ed+_src"])
        )
        ...
        hbalerror = {}
        hbalerror["error"] = (
            hbalsum["R_and_sources_sum"] - hbalsum["sheath_sum"]
        ) / hbalsum["sources_sum"]
    ...
    def get_properties(self):
        ...
        if "sheath_boundary_simple" not in ds.options.keys():
            raise Exception(
                "Extracting gamma only implemented from sheath_boundary_simple"
            )
        ...
        if "sheath_ion_polytropic" in ds.options["sheath_boundary_simple"].keys():
            self.sheath_ion_polytropic = ds.options["sheath_boundary_simple"][
    ...
    def calculate_sheath_fluxes(
        self, method="flux_diagnostic", override_vi=False, calculate_ve=False
    ):
        """
        ...
        cssheath = np.sqrt(
            (sheath_ion_polytropic * tisheath * qe + Zi * tesheath * qe) / Mi
        )  # [m/s] Bohm criterion sound speed

        if calculate_ve:
            ion_sum = Zi * nisheath * cssheath  # Sheath current
            phisheath = tesheath * np.log(
                np.sqrt(tesheath / (Me * 2 * np.pi)) * (1 - Ge) * nesheath / ion_sum
            )  # [V] sheath potential, (note Neumann BC)
            vesheath = (
                np.sqrt(tesheath / (2 * np.pi * Me))
                * (1 - Ge)
                * np.exp(-(phisheath - phi_wall) / tesheath)
            )
        else:
            ion_sum = 0
            phisheath = 0
        ...
        if method == "reconstruction":
            pfi_sheath = (
                -1 * visheath * nisheath * dasheath
            )  # [s^-1] ion particle flux into domain
            pfe_sheath = (
                -1 * vesheath * nesheath * dasheath
            )  # [s^-1] electron particle flux into domain

            hfi_sheath = (
                pfi_sheath * self.gamma_i * tisheath * qe * 1e-6
            )  # [MW] electron heat flux into domain
            hfe_sheath = (
                pfe_sheath * self.gamma_e * tesheath * qe * 1e-6
            )  # [MW] electron heat flux into domain
    ...
    def plot_heat_balance(self):
        ...
        t = self.ds.t
        ax.stackplot(
            t, list(pos.values()), labels=pos.keys(), baseline="zero", alpha=0.7
        )
        ax.stackplot(
            t, list(neg.values()), labels=neg.keys(), baseline="zero", alpha=0.7
        )
    ...
    def plot_flux_balance_old(
        self, use_diagnostics=True, xlims=(None, None), flux_style={}
    ):
        ...
        if use_diagnostics is True:
            ...
            hfe_cond = (
                ds["kappa_par_e"] * np.gradient(ds["Te"], ds["pos"]) * ds["da"] * -1e-6
            )  # * 1e-2
            hfi_cond = (
                ds["kappa_par_d+"]
                * np.gradient(ds["Td+"], ds["pos"])
                * ds["da"]
                * -1e-6
            )  # * 1e-2
        ...
        ax.plot(
            ds["pos"], hfe_cond, c="indigo", label="electron conduction", **flux_style
        )
        ...
        ax.plot(
            ...
            hfe_p_adv,
            c="skyblue",
            label="electron internal energy adv.",
            **flux_style,
        )
        ...
        ax.plot(
            ...
            hfe_k_adv,
            c="skyblue",
            label="electron kinetic energy adv.",
            ls="--",
            **flux_style,
        )
        ...
        ax.plot(
            ds["pos"], src_tot, label="Total source", c="grey", alpha=0.3, lw=4, ls=":"
        )
    ...
    def plot_flux_balance2_old(self, xlims=(None, None), title=""):
        ...
        sources = [
            ("Pd+_src", "Ion external source", 3.0 / 2),
            ("Pe_src", "Electron external source", 3.0 / 2),
        ...
        for name, label in flows:
            ...
            flow_data[name] = (this_flow, label)
        ...
        flow_data["total"] = (total_flow, "Total flow")
        ...
        for name, label, factor in sources:
            ...
            source_data[name] = (this_source, label)
            total_source += this_source
        source_data["total"] = (total_source, "Total sources")
    ...
    def print_balances2(self):
        ...
        print(
            f"Total ion particle source:  {total_ion_source:.3e}  (check: {particle_source + ionization + recombination + feedback_source:.3e})"
        )
        ...
        print(
            f"Total neutral particle source: {total_neutral_source:.3e} (check: {neutral_source - ionization - recombination + neutral_recycle:.3e})"
        )
        ...
        print(
            f"Total power loss: {(ion_energy_flux + electron_energy_flux - recycle_heating - R_ex - R_rec):.3e} MW"
        )
        print(f"  |- Ions:              {ion_energy_flux:.3e} MW")
        print(f"      |- Convection          {ion_convection:.3e} MW")
        print(f"      |- Kinetic energy      {ion_kinetic:.3e} MW")
        print(
            f"      |- Conduction          {(ion_energy_flux - ion_kinetic - ion_convection):.3e} MW"
        )
        ...
        print(
            f"      |- Conduction          {(electron_energy_flux - electron_kinetic - electron_convection):.3e} MW"
        )
...
class FluxBalance:
    def __init__(self, ds, include_neutrals=True):
        ...
        domp = self.domp = dom.shift(
            pos=-1
        )  # # Shift to get the next point in the domain
    ...
    def plot_total_balance(self):
        ...
        if self.include_neutrals:
            ...
            radiation[-1] = (
                0  # Domain has one guard cell where volumetric quantities are zero
            )
