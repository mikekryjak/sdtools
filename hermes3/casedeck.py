...
class CaseDeck1D:
    ...
    def get_stats(self):
        ...
        for casename in self.casenames:
            ...
            self.stats.loc[casename, "target_flux"] = (
                case.ds["NVd+"].isel(t=-1).values[-2]
            )
            self.stats.loc[casename, "target_temp"] = (
                case.ds["Te"].isel(t=-1).values[-2]
            )

            self.stats.loc[casename, "initial_dens"] = (
                case.ds.options["Nd+"]["function"] * case.ds.metadata["Nnorm"]
            )
        ...
        for list_params in vars:
            ...
            for i, ax in enumerate(axes):
                ...
                for i, case in enumerate(self.casenames):
                    ds = self.cases[case].ds.isel(
                        t=-1, pos=slice(2, -1)
                    )  # Final timestep, skip outer guards
                ...
                ax.set_ylabel(
                    "{} ({})".format(ds[param].name, ds[param].units), fontsize=11
                )
