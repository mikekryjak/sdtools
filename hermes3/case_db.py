...
class CaseDB:
    ...
    def __init__(self, case_dir=None, grid_dir=None):

        # Set default paths
        onedrive_path = onedrive_path = (
            str(os.getcwd()).split("OneDrive")[0] + "OneDrive"
        )
        case_dir = (
            os.path.join(onedrive_path, r"Project\collab\tech\cases")
            if case_dir == None
            else case_dir
        )
        grid_dir = (
            os.path.join(onedrive_path, r"Project\collab\tech\grid")
            if grid_dir == None
            else grid_dir
        )
    ...
    def get_grid_path(self, casename):
        ...
        options = BoutOptionsFile(
            os.path.join(casepath, "BOUT.inp"), recalculate_xyz=False
        )
    ...
    def load_case_2D(self, casename, **kwargs):

        casepath = self.casepaths[casename]
        gridfilepath = self.get_grid_path(casename)

        return Load.case_2D(casepath=casepath, gridfilepath=gridfilepath, **kwargs)
