...
class Mesh:
    ...
    def slices(self, name):
        ...
        def upper_seam(i):
            ...
            return (
                slice(0 + MXG, 1 + MXG),
                np.r_[
                    slice(j1_2g + 1, j1_2g + (i + 1)), slice(j2_1g + 1 - (i), j2_1g + 1)
                ],
            )
        ...
        def lower_seam(i):
            ...
            return (
                slice(0 + MXG, 1 + MXG),
                np.r_[slice(j2_2g + 1 - i, j2_2g + 1), slice(j1_1g + 1, j1_1g + 1 + i)],
            )
        ...
        def custom_core_ring(i):
            ...
            return (
                ...
                np.r_[
                    slice(self.j1_2g + 1, self.j2_2g + 1),
                    slice(self.j1_1g + 1, self.j2_1g + 1),
                ],
            )
        ...
        def symmetric_puff(width, center_half_gap):
            ...
            selection = (
                ...
                np.r_[
                    slice(
                        midplane_b + center_half_gap,
                        midplane_b + center_half_gap + width,
                    ),
                    slice(
                        midplane_b - center_half_gap - width,
                        midplane_b - center_half_gap,
        ...
        slices["all_noguards"] = (
            slice(self.MXG, -self.MXG),
            np.r_[
                slice(self.MYG, self.ny_inner - self.MYG * 2),
                slice(self.ny_inner + self.MYG * 3, self.nyg - self.MYG),
            ],
        )

        slices["inner_core"] = (
            slice(0, self.ixseps1),
            np.r_[
                slice(self.j1_1g + 1, self.j2_1g + 1),
                slice(self.j1_2g + 1, self.j2_2g + 1),
            ],
        ...
        slices["outer_core_edge"] = (
            slice(0 + self.MXG, 1 + self.MXG),
            slice(self.j1_2g + 1, self.j2_2g + 1),
        )
        slices["inner_core_edge"] = (
            slice(0 + self.MXG, 1 + self.MXG),
            slice(self.j1_1g + 1, self.j2_1g + 1),
        )
        slices["core_edge"] = (
        ...
        slices["outer_sol_edge"] = (
            slice(-1 - self.MXG, -self.MXG),
            slice(self.ny_inner + self.MYG * 3, self.nyg - self.MYG),
        )
        slices["inner_sol_edge"] = (
            slice(-1 - self.MXG, -self.MXG),
            slice(self.MYG, self.ny_inner + self.MYG),
        )
        ...
        slices["inner_lower_target"] = (
            slice(None, None),
            slice(self.MYG, self.MYG + 1),
        )
        slices["inner_upper_target"] = (
            slice(None, None),
            slice(self.ny_inner + self.MYG - 1, self.ny_inner + self.MYG),
        )
        slices["outer_upper_target"] = (
            slice(None, None),
            slice(self.ny_inner + self.MYG * 3, self.ny_inner + self.MYG * 3 + 1),
        )
        slices["outer_lower_target"] = (
            slice(None, None),
            slice(self.nyg - self.MYG - 1, self.nyg - self.MYG),
        )

        slices["inner_lower_target_guard"] = (
            slice(None, None),
            slice(self.MYG - 1, self.MYG),
        )
        slices["inner_upper_target_guard"] = (
            slice(None, None),
            slice(self.ny_inner + self.MYG, self.ny_inner + self.MYG + 1),
        )
        slices["outer_upper_target_guard"] = (
            slice(None, None),
            slice(self.ny_inner + self.MYG * 3 - 1, self.ny_inner + self.MYG * 3),
        )
        slices["outer_lower_target_guard"] = (
            slice(None, None),
            slice(self.nyg - self.MYG, self.nyg - self.MYG + 1),
        )

        slices["inner_lower_pfr"] = (slice(0, self.ixseps1), slice(None, self.j1_1g))
        slices["outer_lower_pfr"] = (
            slice(0, self.ixseps1),
            slice(self.j2_2g + 1, self.nyg),
        )
        ...
        slices["lower_pfr_edge"] = (
            slice(self.MXG, self.MXG + 1),
            np.r_[slice(None, self.j1_1g + 1), slice(self.j2_2g + 1, self.nyg)],
        )
        slices["upper_pfr_edge"] = (
            slice(self.MXG, self.MXG + 1),
            slice(self.j2_1g + 1, self.j1_2g + 1),
        )
        slices["pfr_edge"] = (
        ...
        slices["outer_midplane_a"] = (
            slice(None, None),
            int((self.j2_2g - self.j1_2g) / 2) + self.j1_2g,
        )
        slices["outer_midplane_b"] = (
            slice(None, None),
            int((self.j2_2g - self.j1_2g) / 2) + self.j1_2g + 1,
        )

        slices["inner_midplane_a"] = (
            slice(None, None),
            int((self.j2_1g - self.j1_1g) / 2) + self.j1_1g + 1,
        )
        slices["inner_midplane_b"] = (
            slice(None, None),
            int((self.j2_1g - self.j1_1g) / 2) + self.j1_1g,
        )
    ...
    def plot_slice(self, slice, dpi=100):
        ...
        fig, axes = plt.subplots(
            1, 3, figsize=(12, 5), dpi=dpi, gridspec_kw={"width_ratios": [2.5, 1, 2]}
        )
    ...
    def plot_xy_grid(self, ax):
        ...
        ax.plot(
            [self.yflat[self.j1_1g]] * np.ones_like(self.xflat),
            self.xflat,
            label="j1_1g",
            color=self.colors[0],
        )
        ax.plot(
            [self.yflat[self.j1_2g]] * np.ones_like(self.xflat),
            self.xflat,
            label="j1_2g",
            color=self.colors[1],
        )
        ax.plot(
            [self.yflat[self.j2_1g]] * np.ones_like(self.xflat),
            self.xflat,
            label="j2_1g",
            color=self.colors[2],
        )
        ax.plot(
            [self.yflat[self.j2_2g]] * np.ones_like(self.xflat),
            self.xflat,
            label="j2_2g",
            color=self.colors[3],
        )
        ...
        ax.plot(
            self.yflat,
            [self.yflat[self.ixseps2]] * np.ones_like(self.yflat),
            label="ixseps1",
            color=self.colors[5],
            ls=":",
        )
    ...
    def plot_rz_grid(
        ...
        if plot_region_borders is True:
            ...
            ax.plot(
                self.Rxy[self.ixseps1, :],
                self.Zxy[self.ixseps1, :],
                label="ixseps1",
                color=self.colors[4],
                alpha=0.7,
                lw=1,
            ...
            ax.plot(
                self.Rxy[self.ixseps2, :],
                self.Zxy[self.ixseps2, :],
                label="ixseps2",
                color=self.colors[5],
                alpha=0.7,
                lw=1,
    ...
    def plot_field(self, name):
        ...
        fig, axes = plt.subplots(
            1, 3, figsize=(10, 6), gridspec_kw={"width_ratios": [5, 2.0, 0.3]}, dpi=110
        )
        fig.subplots_adjust(wspace=0.3)
        fig.suptitle(name)
    ...
    def write_field(self, field, dtype="Field3D"):
        ...
        if field.name in self.mesh.list():
            print(
                f">>> Field {field.name} already exists in {self.filepath}, it will be overwritten"
            )
...
class Field:
    def __init__(self, name, mesh):
        ...
        self.data = np.zeros_like(
            self.mesh.Rxy
        )  # Copy any array from existing grid as a template
    ...
    def set_value(self, region, value, make_per_volume=True):
        ...
        if make_per_volume is True:
            value = value * (
                cell_volumes / total_volume
            )  # Split between all the cells in region according to their volumes
            value = value / cell_volumes  # Make on a per volume basis

        self.data[region] = value

    def plot(
        self, dpi=80, cmap="YlOrRd", xlims=(None, None), ylims=(None, None), legend=True
    ):
        ...
        fig, axes = plt.subplots(
            1, 3, figsize=(10, 6), gridspec_kw={"width_ratios": [5, 2.0, 0.3]}, dpi=dpi
        )
        fig.subplots_adjust(wspace=0.3)
