...
def get_Kei_coll(ds):
    ...
    if "t" in ds.sizes:
        raise ValueError(
            "Dataset must have only one time slice for this function to work correctly."
        )
    ...
    for i in range(len(Te)):
        if Te[i] < 0.1 or Ni[i] < 1e10 or Ne[i] < 1e10:
            ...
            coulomb_log[i] = (
                23.0 - 0.5 * np.log(Ni[i]) + 1.5 * np.log(Ti[i]) - np.log(Zi**2 * Ai)
            )
        elif Te[i] < np.exp(2) * Zi**2:
            coulomb_log[i] = (
                30.0 - 0.5 * np.log(Ne[i]) - np.log(Zi) + 1.5 * np.log(Te[i])
            )
        else:
            coulomb_log[i] = 31.0 - 0.5 * np.log(Ne[i]) + np.log(Te[i])

    vesq = 2 * softFloor(Te, 0.1) * qe / Me
    visq = 2 * softFloor(Ti, 0.1) * qe / (Mp * Ai)
    nu = (
        (qe**2 * Zi) ** 2
        * np.clip(Ni, a_min=0.0, a_max=None)
        * softFloor(coulomb_log, 1.0)
        * (1.0 + me_mi)
        / (3 * (np.pi * (vesq + visq)) ** 1.5 * (e0 * Me) ** 2)
        * ei_multiplier
    )
