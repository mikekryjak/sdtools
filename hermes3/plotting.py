...
class Monitor:
    def __init__(self, case, windows, settings=None, save=False):
        ...
        self.omp = case.ds.hermesm.select_region("outer_midplane_a").isel(
            x=slice(m["MYG"], -m["MYG"])
        )
        ...
        for row_id in range(num_rows):
            ...
            fig, self.axes = plt.subplots(
                1,
                num_windows,
                figsize=(self.fig_size * num_windows * 1.2, self.fig_size),
            )
    ...
    def add_plot(self, ax, name):
        ...
        if name == "target_temp":
            ...
            for target in ["outer_lower"]:
                ...
                ax.plot(
                    self.ds["t"],
                    targets[target].peak_temperature,
                    label=target,
                    color=self.c[0],
                )
            ...
            self.core["Ne"].mean(["x", "theta"]).plot(
                ax=ax, label="Ne core", ls="--", c=self.c[0]
            )
            self.core["Nd"].mean(["x", "theta"]).plot(
                ax=ax, label="neut core", ls="--", c=self.c[1]
            )
            self.sol["Ne"].mean(["x", "theta"]).plot(ax=ax, label="Ne sol", c=self.c[0])
            self.sol["Nd"].mean(["x", "theta"]).plot(
                ax=ax, label="neut sol", c=self.c[1]
            )
            ax.legend(
                fontsize=9, loc="upper center", bbox_to_anchor=(0.5, 1.35), ncol=2
            )
            ax.set_yscale("log")

        elif name == "avg_temp":
            self.core["Td+"].mean(["x", "theta"]).plot(
                ax=ax, label="Td+ core", ls="--", c=self.c[0]
            )
            self.core["Te"].mean(["x", "theta"]).plot(
                ax=ax, label="Te core", ls="--", c=self.c[1]
            )
            self.sol["Td+"].mean(["x", "theta"]).plot(
                ax=ax, label="Td+ sol", c=self.c[0]
            )
            ...
            (self.core["Rd+_ex"].mean(["x", "theta"]) * -1).plot(
                ax=ax, label="core", c=self.c[0]
            )
            (self.sol["Rd+_ex"].mean(["x", "theta"]) * -1).plot(
                ax=ax, label="sol", c=self.c[1]
            )
            ax.legend(
                fontsize=9, loc="upper center", bbox_to_anchor=(0.5, 1.35), ncol=2
            )
            ...
            self.core["Sd+_iz"].mean(["x", "theta"]).plot(
                ax=ax, label="core", c=self.c[0]
            )
            self.sol["Sd+_iz"].mean(["x", "theta"]).plot(
                ax=ax, label="sol", c=self.c[1]
            )
            ax.legend(
                fontsize=9, loc="upper center", bbox_to_anchor=(0.5, 1.35), ncol=2
            ...
            abs(self.core["Sd+_rec"].mean(["x", "theta"])).plot(
                ax=ax, label="core", c=self.c[0]
            )
            abs(self.sol["Sd+_rec"].mean(["x", "theta"])).plot(
                ax=ax, label="sol", c=self.c[1]
            )
            ax.legend(
                fontsize=9, loc="upper center", bbox_to_anchor=(0.5, 1.35), ncol=2
            ...
            """
            omp = self.case.ds.hermesm.select_region("outer_midplane_a").sel(
                t=slice(None, None)
            )
            ...
            (omp["Td+"].isel(x=a) - omp["Td+"].isel(x=b)).plot.line(
                ax=ax, x="t", label="Ions", c=self.c[0]
            )
            (omp["Te"].isel(x=a) - omp["Te"].isel(x=b)).plot.line(
                ax=ax, x="t", label="Electrons", c=self.c[1]
            )
            ...
            (omp["Ne"].isel(x=a) - omp["Ne"].isel(x=b)).plot.line(
                ax=ax, x="t", c=self.c[0], label="Ne"
            )
            ...
            ax.plot(
                self.ds.coords["t"],
                self.ds.data_vars["cvode_last_order"].values,
                label="last_order",
                lw=1,
                c=self.c[0],
            )
            ...
            ax.plot(
                self.ds.coords["t"], cvode["num_fails"], c=self.c[0], label="num_fails"
            )
            ax.plot(
                self.ds.coords["t"],
                cvode["nonlin_fails"],
            ...
            ncalls_per_timestep = self.ds["ncalls"].data[0:-1] / self.ds.coords[
                "t"
            ].diff("t")
            ax.plot(
                self.ds.coords["t"][0:-1],
                ncalls_per_timestep,
                c=self.c[0],
                lw=1,
                markersize=1,
            ...
            ax.plot(
                self.ds.coords["t"],
                np.divide(self.ds["cvode_nliters"], self.ds["cvode_nniters"]),
                c=self.c[0],
                lw=1,
                markersize=1,
                marker="o",
            ...
            ax.plot(
                self.ds.coords["t"],
                self.ds["cvode_npevals"] / self.ds["cvode_nliters"],
                c=self.c[0],
                lw=1,
                markersize=1,
                marker="o",
            ...
            ax.plot(
                self.ds.coords["t"],
                (self.ds["cvode_num_fails"] + self.ds["cvode_nonlin_fails"])
                / self.ds["cvode_nsteps"],
                c=self.c[0],
                lw=1,
                markersize=1,
                marker="o",
...
class Monitor2D:
    ...
    def __init__(self, case, mode, windows, settings=None):
        ...
        if mode == "grid":
            ...
            self.fig_height = (
                0.8 * self.fig_size * self.settings["all"]["figure_aspect"]
            )
            self.wspace = 0.3
            ...
            self.fig_height = (
                1.8 * self.fig_size * self.settings["all"]["figure_aspect"]
            )
    ...
    def add_plot(self, ax, name):
        ...
        self.settings[name] = {
            "log": False,
            "vmin": self.ds[name].min().values,
            "vmax": self.ds[name].max().values,
        }
        ...
        if self.mode == "grid":
            norm = create_norm(
                logscale=settings["log"],
                norm=None,
                vmin=settings["vmin"],
                vmax=settings["vmax"],
            )
            abs(self.ds[name]).plot(
                ax=ax,
                cmap="Spectral_r",
                cbar_kwargs={"label": ""},
                vmin=settings["vmin"],
            ...
            self.case.ds.hermesm.select_region("outer_midplane_a")[name].plot(
                x="t", ax=ax, cmap="Spectral_r", norm=norm, cbar_kwargs={"label": ""}
            )
            ...
            self.case.ds.hermesm.select_region("outer_lower_target")[name].plot(
                x="t", ax=ax, cmap="Spectral_r", norm=norm, cbar_kwargs={"label": ""}
            )
            ...
            self.case.select_custom_sol_ring("outer_lower", sepadd=1)[name].plot(
                x="t", ax=ax, cmap="Spectral_r", norm=norm, cbar_kwargs={"label": ""}
            )
...
def plot_ddt(
    ...
    for param in list_params:
        ...
        if case.is_2d:
            res[param] = np.sqrt(
                np.mean(res[param] ** 2, axis=(1, 2))
            )  # Root mean square
        else:
            res[param] = np.sqrt(np.mean(res[param] ** 2, axis=1))  # Root mean square
        res[param] = np.convolve(
            res[param], np.ones(smoothing), "same"
        )  # Moving average with window = smoothing
...
def plot_monitors(self, to_plot, what=["mean", "max", "min"], ignore=[]):
    ...
    if to_plot == "pressure":
        ...
        for var in self.ds.data_vars:
            if "N" in var and not any(
                [x in var for x in ignore + ["S", ")", "_", "V"]]
            ):
                list_params.append(var)
...
def diagnose_cvode(ds, lims=(0, 0), scale="log"):
    ...
    ax.plot(
        ds.coords["t"],
        ds.data_vars["cvode_last_order"].values,
        label="last_order",
        lw=1,
    )
    ...
    ax.plot(
        ds.coords["t"], ds.data_vars["cvode_nonlin_fails"].values, label="nonlin_fails"
    )
...
def plot_selection(ds, selection, dpi=100, rz_only=False, show_selection=True):
    ...
    fig.subplots_adjust(wspace=0.15, top=0.75, bottom=0.05, left=0.10, right=0.95)
...
def plot_xy_grid(ds, ax):
    ...
    ax.plot(
        [yflat[m["j1_1g"]]] * np.ones_like(xflat),
        xflat,
        label="j1_1g",
        color=m["colors"][0],
    )
    ax.plot(
        [yflat[m["j1_2g"]]] * np.ones_like(xflat),
        xflat,
        label="j1_2g",
        color=m["colors"][1],
    )
    ax.plot(
        [yflat[m["j2_1g"]]] * np.ones_like(xflat),
        xflat,
        label="j2_1g",
        color=m["colors"][2],
    )
    ax.plot(
        [yflat[m["j2_2g"]]] * np.ones_like(xflat),
        xflat,
        label="j2_2g",
        color=m["colors"][3],
    )
    ax.plot(
        yflat,
        [yflat[m["ixseps1"]]] * np.ones_like(yflat),
        label="ixseps1",
        color=m["colors"][4],
        lw=1,
    )
    if m["topology"] != "single-null":
        ax.plot(
            yflat,
            [yflat[m["ixseps2"]]] * np.ones_like(yflat),
            label="ixseps1",
            color=m["colors"][5],
            ls=":",
            lw=1,
    ...
    ax.plot(
        yflat[m["ny_inner"]] * np.ones_like(xflat),
        xflat,
        label="ny_inner",
        color=m["colors"][5],
    )
...
def lineplot(
    ...
    if combine_regions:
        ...
        fig.subplots_adjust(
            hspace=0.5, wspace=0.3, bottom=0.15, left=0.1, right=0.9, top=0.9
        )
        figure_regions = [
            (fig, axes_grid[row_index], region)
    ...
    for fig, axes, region in figure_regions:
        ...
        for name in cases.keys():
            ...
            if region == "omp":
                ...
                dfs[name] = get_1d_poloidal_data(
                    ds, params, "outer_lower_sol", sepadd=1
                )
            ...
            for j, name in enumerate(cases.keys()):
                if region == "1d":
                    ...
                    if param in region_ds[name].data_vars:
                        data = region_ds[name][
                            param
                        ].values.copy()  # Extract numpy array
                    else:
        ...
        for j, name in enumerate(cases.keys()):
            legend_items.append(
                mpl.lines.Line2D([0], [0], color=colors[j], lw=2, ls=ls)
            )
        ...
        if not combine_regions:
            ...
            if save_name != "":
                fig.savefig(
                    f"{save_name}_{region}.png", bbox_inches="tight", pad_inches=0.2
                )
...
def plot_perp_heat_fluxes(
    ...
    if neutrals_only is False:
        ax.plot(
            dist,
            append_rhs("hf_perp_diff_L_e") * scale,
        ...
        ax.plot(
            dist,
            append_rhs("hf_perp_diff_L_d+") * scale,
            label="Ion conduction",
            marker=m,
            ms=ms,
            ls="--",
            c="red",
        ...
        ax.plot(
            dist,
            append_rhs("hf_perp_conv_L_d+") * scale,
            label="Ion convection",
            marker="x",
            ls=":",
            ms=ms,
            c="red",
        ...
        ax.plot(
            dist,
            append_rhs("hf_perp_tot_L_d+") * scale,
            label="Ion total",
            marker="|",
            ls="-",
            ms=0,
...
def plot_particle_balance(ds, ylims=(None, None)):
    ...
    ax.plot(
        ds.coords["t"], ds["pf_int_total_net"], lw=2, ls="--", c="k", label="Imbalance"
    )
...
def plot_heat_balance(ds, ylims=(None, None)):
    ...
    ax.stackplot(
        ...
        labels=labels_neg,
        baseline="zero",
        colors=["darkorange", "deeppink", "crimson"],
        alpha=0.7,
    )

    ax.plot(
        ds.coords["t"], ds["hf_int_total_net"], lw=2, ls="--", c="k", label="Imbalance"
    )
    ax.set_xlabel("Time [s]")
...
def plot_omp(da_list, legend=True, title=True, dpi=150, **kwargs):
    ...
    for da in da_list:
        da.hermesm.select_region("outer_midplane_a").plot(
            ax=ax, label=da.standard_name, **kwargs
        )
...
def plot_density_feedback_controller(ds):
    ...
    ds["density_feedback_src_mult_d+"].plot(
        ax=axes[1], c=colors[0], ls=":", label="Total"
    )
...
def plot_2d_timeslices(
    ...
    **kwargs,
    ...
    if vmin == None:
        vmin = np.nanmin(mins)
    if vmax == None:
        vmax = np.nanmax(maxes)

    # Make norm and get settings
    norm = create_norm(logscale, None, vmin, vmax)
    style = dict(
        cmap=cmap, antialias=True, linewidth=0.1, linecolor="k", add_colorbar=False
    )
    ...
    for i, tind in enumerate(tinds):
        if common_scale is True:
            ds.isel(t=tind)[param].hermesm.clean_guards().bout.polygon(
                ax=axes[i], norm=norm, **style
            )
        else:
            ds.isel(t=tind)[param].hermesm.clean_guards().bout.polygon(
                ax=axes[i], vmin=None, vmax=None, **style
            )
    ...
    cbar = plt.colorbar(
        mappable=sm, cax=cax, label=param
...
def plot2d(
    ...
    **kwargs,
    ...
    for i, inputs in enumerate(toplot):
        ...
        if clear_guards == True:
            data = data.hermes.clear_guards()
...
def plot_cvode_performance_single(ds):
    ...
    if "wtime" in ds:
        ...
        print(
            "WARNING: wtime not found in dataset, ensure you use the right xBOUT version"
        )
        ms_per_24hrs = 0
...
def plot_fluctuations(
    ...
    for i, param in enumerate(params):
        ...
        ax = axes[i]
        data.hermes.clear_guards().bout.polygon(
            ax=ax,
            cmap="Spectral_r",
            targets=False,
...
def plot_variable_magnitude(ds, ax=None, ylims=None):
    ...
    bplot = ax.boxplot(
        ...
        patch_artist=True,
        whis=(5, 95),
        flierprops={"ms": 1, "markerfacecolor": "k", "color": "grey", "mew": 0.2},
    )
