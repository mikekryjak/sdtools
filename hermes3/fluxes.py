...
def calculate_simple_particle_balance(ds):
    ...
    s["pump_recycle"] = (
        ((domain["Sd_pump"]) * domain["dv"]).sum(["x", "theta"])
        * m["Nnorm"]
        * m["Omega_ci"]
    )
    s["wall_recycle"] = (
        ((domain["Sd_wall_recycle"]) * domain["dv"]).sum(["x", "theta"])
        * m["Nnorm"]
        * m["Omega_ci"]
    )
...
def calculate_simple_heat_balance(ds, verbose=True):
    ...
    if "Ed_wall_refl" in ds.data_vars:
        for place in places:
            ...
            hflows[place] = ()
            hflows[place] = (
                ds_place["Ed_wall_refl"] * ds_place["dv"]
            ).sum().values * 1e-6
...
def calculate_neutral_heat_balance(ds, mode="default"):
    ...
    def get_bndry_flux(species, boundary, type="convection"):
        ...
        if param in ds.data_vars:
            return (
                ds.hermesm.select_region("core_edge")[param].sum("theta").values * 1e-6
            )
        else:
    ...
    df.loc["TOTAL BALANCE", :] = df[~df.index.isin(["target_trefl", "wall_trefl"])].sum(
        axis=0
    )

    if mode == "reflection":
        dfrefl = df.transpose()[
            ["outer_sol_refl", "inner_sol_refl", "target_refl", "pfr_refl"]
        ].copy()
        dfrefl = dfrefl.rename(
            columns={
                "outer_sol_refl": "outer_wall",
                "inner_sol_refl": "inner_wall",
                "target_refl": "targets",
                "pfr_refl": "pfr",
            }
        )
...
def calculate_heat_balance(ds, merge_targets=True):
    ...
    for species in m["species"]:
        ds[f"hf_int_src_{species}"] = (
            (domain[f"P{species}_src"] * domain["dv"]).sum(["x", "theta"]).squeeze()
            * 3
            / 2
        )
        ds[f"hf_int_core_{species}"] = (
            core[f"hf_perp_tot_L_{species}"].sum("theta").squeeze()
        )
        ds[f"hf_int_sol_{species}"] = (
            sol[f"hf_perp_tot_R_{species}"].sum("theta").squeeze() * -1
        )  # Positive flow at SOL leaves model
        ds[f"hf_int_pfr_{species}"] = (
            pfr[f"hf_perp_tot_L_{species}"].sum("theta").squeeze()
        )
    ...
    for species in m["ion_species"] + m["neutral_species"] + ["e"]:
        ...
        for target in m["targets"]:
            target_name = f"{target}_target"
            ds[f"hf_int_{target_name}_{species}"] = (
                ds[f"hf_{target_name}_{species}"].sum("x").squeeze()
            )
    ...
    for ion in m["ion_species"]:
        if f"R{ion}_ex" in domain.data_vars:
            ds[f"hf_int_rad_ex_e"] = (
                (domain[f"R{ion}_ex"] * domain["dv"]).sum(["x", "theta"]).squeeze()
            )
            net_ex += ds[f"hf_int_rad_ex_e"]

        if f"R{ion}_rec" in domain.data_vars:
            ds[f"hf_int_rad_rec_e"] = (
                (domain[f"R{ion}_rec"] * domain["dv"]).sum(["x", "theta"]).squeeze()
            )
    ...
    ds[f"hf_int_total_net"] = (
        ...
        + ds["hf_int_sol_net"]
        + ds["hf_int_pfr_net"]
        + ds["hf_int_targets_net"]
        + ds["hf_int_rad_ex_net"]
        + ds["hf_int_rad_rec_net"]
    )
...
def calculate_particle_balance(ds):
    ...
    for species in m["ion_species"] + m["neutral_species"]:
        ds[f"pf_int_src_{species}"] = (
            (domain[f"S{species}_src"] * domain["dv"]).sum(["x", "theta"]).squeeze()
        )
        ds[f"pf_int_core_{species}"] = (
            core[f"pf_perp_diff_L_{species}"].sum("theta").squeeze()
        )
        ds[f"pf_int_sol_{species}"] = (
            sol[f"pf_perp_diff_R_{species}"].sum("theta").squeeze() * -1
        )  # Positive is going right, which is flowing OUT
        ds[f"pf_int_pfr_{species}"] = (
            pfr[f"pf_perp_diff_L_{species}"].sum("theta").squeeze()
        )
    ...
    for species in m["ion_species"] + m["neutral_species"]:
        net[f"targets_{species}"] = 0
        for target_name in m["targets"]:
            ds[f"pf_int_{target_name}_{species}"] = (
                ds[f"pf_{target_name}_{species}"].sum("x").squeeze()
            )
    ...
    for ion in m["ion_species"]:
        if f"S{ion}_iz" in domain.data_vars:
            neutral = ion.split("+")[0]
            ds[f"pf_int_iz_{ion}"] = (
                (domain[f"S{ion}_iz"] * domain["dv"]).sum(["x", "theta"]).squeeze()
            )
            ds[f"pf_int_iz_{neutral}"] = ds[f"pf_int_iz_{ion}"] * -1

        if f"S{ion}_rec" in domain.data_vars:
            ds[f"pf_int_rec_{ion}"] = (
                (domain[f"S{ion}_rec"] * domain["dv"]).sum(["x", "theta"]).squeeze()
            )
...
def calculate_target_fluxes(ds):
    ...
    if "recycling" in ds.options.keys():
        ...
        ion = m["ion_species"][
            0
        ]  # TODO: Fix this - currently hardcoding the first ion species...
        recycling = False
    ...
    for name in m["targets"]:
        ...
        # targetname = name
        (
            ds[f"hf_{targetname}_e"],
            ds[f"hf_{targetname}_{ion}"],
            ds[f"pf_{targetname}_{ion}"],
        ) = sheath_boundary_simple(ds, ion, target=name)
        ...
        if "Sd_target_recycle" in ds.data_vars:
            ...
            for name in m["targets"]:
                ...
                for species in m["recycle_pair"].values():
                    ...
                    ds[f"pf_recycle_{targetname}_{species}"] = (
                        target[f"S{species}_target_recycle"] * target["dv"]
                    )  # [s-1]
                    ds[f"hf_recycle_{targetname}_{species}"] = (
                        target[f"E{species}_target_recycle"] * target["dv"]
                    )  # [W]
            ...
            for name in m["targets"]:
                ...
                ds[f"pf_recycle_{targetname}_{neutral}"] = (
                    -ds[f"pf_{targetname}_{ion}"] * ds.options[ion][option]
                )  # TODO generalise and check
                ...
                ds[f"hf_recycle_{targetname}_{neutral}"] = xr.zeros_like(
                    ds[f"hf_{targetname}_{ion}"]
                )
...
def calculate_radial_fluxes(ds, force_neumann=False, new_afn=True):
    ...
    for name in m["charged_species"]:
        ...
        L, R = Div_a_Grad_perp_upwind_fast(
            ds,
            ds[f"N{name}"] * ds[f"anomalous_Chi_{name}"],
            constants("q_e") * ds[f"T{name}"],
        )
        ...
        L, R = Div_a_Grad_perp_upwind_fast(
            ds,
            constants("q_e") * ds[f"T{name}"] * ds[f"anomalous_D_{name}"],
            ds[f"N{name}"],
        )
        ...
        # Total
        ds[f"hf_perp_tot_L_{name}"] = (
            ds[f"hf_perp_conv_L_{name}"] + ds[f"hf_perp_diff_L_{name}"]
        )
        ds[f"hf_perp_tot_R_{name}"] = (
            ds[f"hf_perp_conv_R_{name}"] + ds[f"hf_perp_diff_R_{name}"]
        )
    ...
    for name in m["neutral_species"]:
        ...
        if new_afn is True and f"particle_flux_factor_{name}" in ds.data_vars:
            corr = ds[
                f"particle_flux_factor_{name}"
            ]  # Convection limited by particle diffusion factor
        else:
            corr = 1
        ...
        if new_afn is True and f"heat_flux_factor_{name}" in ds.data_vars:
            corr = ds[
                f"heat_flux_factor_{name}"
            ]  # Conduction limited by heat flux factor
        else:
            corr = 1
    ...
    for name in m["neutral_species"] + m["charged_species"]:
        ...
        for side in ["L", "R"]:
            for kind in ["diff", "conv", "tot"]:
                ...
                kind_long = {"diff": "diffusive", "conv": "convective", "tot": "total"}[
                    kind
                ]
                side_long = {"L": "LHS cell face", "R": "RHS cell face"}[side]

                da.attrs["long_name"] = (
                    f"Perpendicular {kind_long} heat flux on {side_long} ({name})"
                )
    ...
    for name in m["charged_species"]:
        L, R = Div_a_Grad_perp_upwind_fast(
            ds, ds[f"anomalous_D_{name}"], ds[f"N{name}"]
        )
    ...
    for name in m["neutral_species"]:
        ...
        corr = (
            ds["particle_flux_factor_d"]
            if "particle_flux_factor_d" in ds.data_vars
            else 1
        )
    ...
    for name in m["neutral_species"] + m["charged_species"]:
        for side in ["L", "R"]:
            for kind in ["diff"]:
                ...
                da.attrs["long_name"] = (
                    f"Perpendicular diffusive particle flux on {side_long} ({name})"
                )
                da.attrs["source"] = "xHermes"
                da.attrs["conversion"] = ""
...
def sheath_boundary_simple(
    bd, species, target, sheath_ion_polytropic=1.0, include_convective=True
):
    """
    ...
    if m["keep_yboundaries"] == 0:
        ...
        target_indices["inner_upper"] = dict(
            y=m["ny_inner"] - 1, y2=m["ny_inner"] - 2, yg=None
        )
        target_indices["outer_upper"] = dict(
            y=m["ny_inner"] + 1, y2=m["ny_inner"] + 2, yg=None
        )
    else:
        target_indices["inner_lower"] = dict(
            y=m["MYG"], y2=m["MYG"] + 1, yg=m["MYG"] - 1
        )
        target_indices["outer_lower"] = dict(
            y=-m["MYG"] - 1, y2=-m["MYG"] - 2, yg=-m["MYG"]
        )
        target_indices["inner_upper"] = dict(
            y=m["ny_inner"] + m["MYG"] - 1,
            y2=m["ny_inner"],
            yg=m["ny_inner"] + m["MYG"],
        )
        target_indices["outer_upper"] = dict(
            y=m["ny_inner"] + m["MYG"] * 3,
            y2=m["ny_inner"] + m["MYG"] * 3 + 1,
            yg=m["ny_inner"] + m["MYG"] * 3 - 1,
        )
    ...
    vesheath = np.where(
        vesheath > abs(Vi.isel(theta=y)), vesheath, abs(Vi.isel(theta=y))
    )
    ...
    if reproduce_error is True:
        if y == -1:
            q_e = (
                (gamma_e * qe * tesheath - 0.5 * me * vesheath**2) * nesheath * vesheath
            )
        else:
            q_e = (
                ((gamma_e - 2.5) * qe * tesheath - 0.5 * me * vesheath**2)
                * nesheath
                * vesheath
            )
    else:
    ...
    hf_e = (
        ...
        * dx.isel(theta=y)
        * dz.isel(theta=y)
        * (J.isel(theta=y) + J.isel(theta=yg))
        / (np.sqrt(g_22.isel(theta=y)) + np.sqrt(g_22.isel(theta=yg)))
    )
    hf_i = (
        q_i
        * dx.isel(theta=y)
...
def sheath_boundary_simple_original(
    bd,
    gamma_e,
    gamma_i,
    Ne,
    ...
    # so gamma_e is the total *energy* flux coefficient.
    q_e = (
        ((gamma_e - 2.5) * qe * tesheath - 0.5 * me * vesheath**2) * nesheath * vesheath
    )
    q_i = gamma_i * qe * tisheath * nesheath * vesheath
...
def Div_a_Grad_perp_upwind_fast(ds, a, f):
    ...
    F_L = flux.shift(
        x=1
    )  # the shift of 1 index to get F_L because the left flux at cell X is the same as the right flux at cell X-1.
