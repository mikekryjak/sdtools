...
def animate_with_reference(ds, params, auxparam):
    ...
    # Plot grid
    gs0a = mpl.gridspec.GridSpec(ncols=num_plots + 1, nrows=1, wspace=wspace)
    ...
    # Make scanning line
    vline = np.repeat(
        np.array([[min(auxparam.values), max(auxparam.values)]]), len(t), axis=0
    )
    ...
    for i, param in enumerate(params):
        ...
        blocks[i] = amp.blocks.Line(
            pos, ds[param].values, ax=axes[i], color="black", marker="o", markersize=1
        )
