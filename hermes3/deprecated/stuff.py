def slicer(self, name):
    ...
    def custom_sol_ring(i, region):
        ...
        if region == "all":
            return (
                slice(i + 1, i + 2),
                np.r_[
                    slice(0 + self.MYG, self.j2_2g + 1),
                    slice(self.j1_1g + 1, self.nyg - self.MYG),
                ],
            )
        ...
        if region == "inner_lower":
            return (
                slice(i + 1, i + 2),
                slice(
                    0 + self.MYG, int((self.j2_1g - self.j1_1g) / 2) + self.j1_1g + 2
                ),
            )
        ...
        if region == "outer_lower":
            return (
                slice(i + 1, i + 2),
                slice(
                    int((self.j2_2g - self.j1_2g) / 2) + self.j1_2g, self.nyg - self.MYG
                ),
            )
        if region == "outer_upper":
            return (
                slice(i + 1, i + 2),
                slice(
                    self.ny_inner + self.MYG * 3,
                    int((self.j2_2g - self.j1_2g) / 2) + self.j1_2g + 2,
                ),
            )
    ...
    slices["inner_core"] = (
        ...
        np.r_[
            slice(self.j1_1g + 1, self.j2_1g + 1), slice(self.j1_2g + 1, self.j2_2g + 1)
        ],
    )
...
def calculate_heat_balance(ds, show=True, merge_targets=True):
    ...
    hf["e"]["rad_rec"] = (
        (domain["Rd+_rec"] * domain["dv"]).sum(["x", "theta"]).squeeze()
    )
    ...
    totals.loc["total(frac)"] = totals.loc["total"] / df.loc[["source", "core"], :].sum(
        axis=0
    )
    ...
    if merge_targets is True:
        ...
        target_total = np.sum(
            [hf_last["d+"][x] + hf_last["e"][x] for x in m["targets"]]
        )
...
def calculate_particle_balance(ds, show=True, merge_targets=True):
    ...
    for species in m["ion_species"]:
        # Atomic reaction fluxes
        pf[species]["iz"] = (
            (domain["Sd+_iz"] * domain["dv"]).sum(["x", "theta"]).squeeze()
        )
        pf[species]["rec"] = (
            (domain["Sd+_rec"] * domain["dv"]).sum(["x", "theta"]).squeeze()
        )
