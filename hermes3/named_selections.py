...
class Target:
    def __init__(self, case, target_name):
        ...
        self.heat_flux = (
            gamma_i * self.temperature * constants("q_e") * self.ion_flux * 1e-6
        )  # MW
    ...
    def plot(self, what):
        ...
        if what == "heat_flux":
            ax.set_title(
                f"{self.case.name}: total heat flux integral: {self.total_heat_flux:,.3f}[MW]"
            )
...
class CoreRing:
    ...
    def extract_rings(self):
        ...
        self.b_slice = self.case.select_custom_core_ring(
            self.ring_index + 1
        )  # Second ring
        ...
        # Geometry properties
        self.dr = (
            self.a["dr"].values / 2 + self.b["dr"].values / 2
        )  # Distance between cell centres of rings
        self.R = (
            self.a["R"].values + self.b["R"].values
        ) / 2  # Major radius of the edge in-between rings
        self.A = (
            (self.a["dpol"].values + self.b["dpol"].values) / 2 * 2 * np.pi * self.R
        )  # Surface area of the edge in-between rings
        self.diff = self.ds.diff(
            dim="x"
        )  # Difference the entire dataset for dN/dT calculations
    ...
    def calculate_fluxes(self):
        ...
        for species in self.list_species:
            ...
            dN[species] = (
                self.ds[f"N{species}"]
                .diff("x")
                .isel(x=self.a_slice[0], theta=self.a_slice[1])
            )
            dT[species] = (
                self.ds[f"T{species}"]
                .diff("x")
            ...
            self.ring_temperature[species] = (
                self.a[f"T{species}"].values + self.b[f"T{species}"].values
            ) / 2  # Temperature [eV]
            self.ring_density[species] = (
                self.a[f"N{species}"].values + self.b[f"N{species}"].values
            ) / 2  # Density [m-3]

            # Calculate flux (D*-dN/dx) in each cell and multiply by its surface area, then sum along the ring
            self.particle_flux[species] = (
                (self.D[species] * -grad_N[species]) * self.A
            ).sum("theta")  # s-1

            # Convective: D*-dN/dx * T
            # Diffusive: Chi*-dT/dx * N
            self.convective_heat_flux[species] = (
                (
                    (self.D[species] * -grad_N[species])
                    * self.A
                    * self.ring_temperature[species]
                ).sum("theta")
                * 3
            ...
            self.diffusive_heat_flux[species] = (
                (
                    (self.Chi[species] * -grad_T[species])
                    * self.A
                    * self.ring_density[species]
                ).sum("theta")
                * 3
            ...
            self.total_heat_flux[species] = (
                self.convective_heat_flux[species] + self.diffusive_heat_flux[species]
            )
