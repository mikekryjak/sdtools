...
class viewer_2d:
    ...
    def __init__(
        ...
        for plot in plots:
            ...
            Rlims["min"].append(plot.Rlim[0])
            Rlims["max"].append(plot.Rlim[1])
            Zlims["min"].append(plot.Zlim[0])
            Zlims["max"].append(plot.Zlim[1])
        ...
        if slider is True:
            slider = RangeSlider(
                fig.add_axes([0.3, 0.05, 0.5, 0.1]),
                "Colour limits",  # left, bottom, width, height
                self.min,
                self.max,
            ...
            def update(val):
                slider.ax.set_ylim(
                    self.min, self.max
                )  # This is inexplicably needed otherwise it freezes
...
class SOLEDGEplot:
    ...
    def __init__(self, path, param):
        ...
        with HiddenPrints():
            self.Plasmas, Plasmas = load_plasma_files(
                path, nZones=0, Evolution=0, iPlasmas=[0, 1]
            )
        ...
        if param == "Pa":
            self.data = (
                self.get_param_data("Nni")
                * self.get_param_data("Tni")
                * constants("q_e")
            )
            ...
            self.data = (
                Plasmas[1][0].Triangles.Values[Ti_i]
                / Plasmas[1][0].Triangles.Values[Te_i]
            )
        else:
    ...
    def __init__(self, path, data=None, param=None):
        ...
        if param != None:
            if param == "Nd":
                self.data = (
                    bal["dab2"][:] + bal["dmb2"][:] * 2
                )  # Add atom and molecular densities
            elif param == "Td":
                self.data = bal["tab2"][:]
            else:
        ...
        for i in range(Nx):
            for j in range(Ny):
                p = mpl.patches.Polygon(
                    np.concatenate([crx[:, j, i][tuple(idx)], cry[:, j, i][tuple(idx)]])
                    .reshape(2, 5)
                    .T,
                    fill=False,
                    closed=True,
    ...
    def __init__(
        ...
        for i, casename in enumerate(cases.keys()):
            ...
            if model["code"] == "hermes":
                ...
                axes[i].set_title(rf"Hermes-3\{casename}")
                ...
                if solps_name != None:
                    ...
                    axes[i].set_title(rf"SOLPS\{casename}")
